...
1. **Start with SQL** (`core/src/.../storage/impl/sql/`) — this is the canonical implementation
2. **Adapt for KafkaSQL** (`core/src/.../storage/impl/kafkasql/`) — uses a Kafka journal pattern;
   state changes are serialized as Kafka messages and replayed
3. **Consider GitOps** (`core/src/.../storage/impl/gitops/`) — file-based, git-backed
4. **Consider KubernetesOps** (`core/src/.../storage/impl/kubernetesops/`) — ConfigMap-based
