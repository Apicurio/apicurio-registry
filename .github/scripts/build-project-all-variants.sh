...
case $BRANCH_NAME in
  ...
  "1.2.x" | "1.3.x")
       ...
       mvn clean install -Pprod -Pasyncmem -Pinfinispan -Pjpa -Pkafka -Pstreams -DskipTests
       ;; 

  "master" | "2.0.x")
       echo "Building Branch '$BRANCH_NAME'"
       echo "Building variants: [mem, asyncmem, infinispan, kafkasql, sql, streams]"
       mvn clean install -Pprod -Pasyncmem -Pinfinispan -Pkafkasql -Psql -Pstreams -pl !tests
       ;;
