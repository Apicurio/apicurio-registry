...
| `-Dkiota.binary.folder` | Where the Kiota generator binary is cached (default: inside the local Maven repository) |
| `-Dkiota.system`      | Use a Kiota already on the system instead of downloading one (default: `false`)          |
| `-Dkiota.skip`        | Skip Kiota generation entirely (default: `false`)                                        |

### Kiota generator binary

The Java SDK is generated by [Kiota](https://github.com/microsoft/kiota), and the build
downloads the generator binary from github.com the first time it needs it. That binary is
cached outside the build tree, in your local Maven repository at
`.cache/kiota-binary/<os.detected.classifier>/<kiota.version>/kiota`, so
`~/.m2/repository/.cache/kiota-binary/linux-x86_64/1.28.0/kiota` for the version pinned as
`kiota.version` in the root pom today. On Windows the file is named `kiota.exe`. Keeping it
there means `mvn clean` no longer forces a fresh download on the next build. The download is
about 35 MB and the binary it extracts about 82 MB.
...
Pointing `-Dkiota.binary.folder` at another absolute path works, and nothing in the build
stops you. `scripts/validate-files.sh`, which CI runs, checks only that the value committed
to the poms is the one above, since moving it back under `target/` makes every build slower
