...
public class StrimziIdentityProvider implements IdentityProvider<UsernamePasswordAuthenticationRequest> {
        ...
        if (authConfig.basicAuthWithStrimziUserEnabled.get()) {
            return context.runBlocking(() -> {
                ...
                boolean passwordCorrect = Optional.ofNullable(getPasswordOfKafkaUser(givenUsername))
                        .map(actualPassword -> actualPassword.equals(givenPassword)).orElse(false);
                if (passwordCorrect) {
                    return QuarkusSecurityIdentity.builder().addCredential(request.getPassword())
                            .setPrincipal(new QuarkusPrincipal(givenUsername)).build();
                } else {
                    throw new AuthenticationFailedException("Invalid username or password");
                }
            });
        } else {
            // return null instead of throwing an exception to allow other identity providers to try to
            // authenticate
    ...
    private String getPasswordOfKafkaUser(String username) {
        ...
        return client.secrets().inNamespace(authConfig.strimziKubernetesNamespace)
                .withLabel("strimzi.io/kind", "KafkaUser").resources()
                .filter(secret -> secret.item().getMetadata().getName().equals(username)).findFirst()
                .map(secret -> secret.item().getData().get("password")).map(Base64.getDecoder()::decode)
