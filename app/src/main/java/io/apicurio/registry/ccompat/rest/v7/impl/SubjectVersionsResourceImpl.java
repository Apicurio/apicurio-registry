...
public class SubjectVersionsResourceImpl extends AbstractResource implements SubjectVersionsResource {
    ...
    public List<Integer> listVersions(String subject, String groupId, Boolean deleted) throws Exception {
        ...
        if (fdeleted) {
            return storage.getArtifactVersions(groupId, subject, DEFAULT).stream().map(VersionUtil::toLong)
                    .map(converter::convertUnsigned).sorted().collect(Collectors.toList());
        } else {
            return storage.getArtifactVersions(groupId, subject, SKIP_DISABLED_LATEST).stream()
    ...
    public SchemaId register(String subject, SchemaInfo request, Boolean normalize, String groupId)
        ...
        if (!idFound) {
            try {
                ...
                final String artifactType = ArtifactTypeUtil.determineArtifactType(
                        ContentHandle.create(request.getSchema()), null, null, resolvedReferences,
                ...
                ArtifactMetaDataDto artifactMeta = createOrUpdateArtifact(subject, request.getSchema(),
        ...
        try {
            if (doesArtifactExist(subject, groupId)) {
                ...
                return VersionUtil.toInteger(parseVersionString(subject, versionString, groupId, version -> {
                    List<Long> globalIdsReferencingSchema = storage.getGlobalIdsReferencingArtifact(groupId,
                            subject, version);
    ...
    private String processDeleteVersion(String subject, String versionString, String groupId, String version,
        ...
        if (fpermanent) {
            if (avmd.getState().equals(ArtifactState.ENABLED)
                    || avmd.getState().equals(ArtifactState.DEPRECATED)) {
