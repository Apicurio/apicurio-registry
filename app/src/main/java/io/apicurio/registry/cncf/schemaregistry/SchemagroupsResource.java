...
public interface SchemagroupsResource {
    ...
    Response getLatestSchema(@PathParam("group-id") String groupId, @PathParam("schema-id") String schemaId);

    /**
     * Register schema. If schema of specified name does not exist in specified group, schema is created at
     * version 1. If schema of specified name exists already in specified group, schema is created at latest
     * version + 1. If schema with identical content already exists, existing schema's ID is returned.
     */
    @Path("/{group-id}/schemas/{schema-id}")
    @POST
    ...
    @Authorized
    SchemaId createSchema(@PathParam("group-id") String groupId, @PathParam("schema-id") String schemaId,
            InputStream data);
    ...
    @GET
    @Produces("application/json;format=avro")
    Response getSchemaVersion(@PathParam("group-id") String groupId, @PathParam("schema-id") String schemaId,
            @PathParam("version-number") Integer versionNumber);

    @Path("/{group-id}/schemas/{schema-id}/versions/{version-number}")
    @DELETE
    @Authorized
    void deleteSchemaVersion(@PathParam("group-id") String groupId, @PathParam("schema-id") String schemaId,
            @PathParam("version-number") Integer versionNumber);
}
