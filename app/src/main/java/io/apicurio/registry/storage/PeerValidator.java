 ...
 * "EU" and "eu" as the same key, while PostgreSQL and H2 treat them as distinct. Rejecting
 * uppercase at validation time keeps create, lookup, update and delete addressing the same
 * peer set on every backend without requiring an explicit collation on every dialect.
 * {@code credentialSecretRef} is not affected; it stays case-sensitive, matching filesystem and
 * Kubernetes Secret key semantics.
 */
public final class PeerValidator {

    private static final Pattern PEER_ID_PATTERN = Pattern.compile("^[a-z0-9._-]{1,256}$");
    ...
    private static void validatePeerId(String peerId) {
        ...
        if (!PEER_ID_PATTERN.matcher(peerId).matches()) {
            throw new InvalidPeerException(
                    "Peer id must contain only lowercase letters, digits, '.', '_' or '-' "
                            + "(max 256 characters).");
        }
        if (".".equals(peerId) || "..".equals(peerId)) {
            throw new InvalidPeerException("Peer id is invalid.");
        }
        if (RESERVED_LOCAL_PEER_ID.equals(peerId)) {
