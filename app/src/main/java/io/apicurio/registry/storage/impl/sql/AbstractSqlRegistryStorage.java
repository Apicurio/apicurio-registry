...
public abstract class AbstractSqlRegistryStorage implements RegistryStorage {
    ...
    public List<ArtifactMetaDataDto> getArtifactVersionsByContentId(long contentId) {
        return handles.withHandleNoException(handle -> {
            List<ArtifactMetaDataDto> dtos = handle
                    .createQuery(sqlStatements().selectArtifactVersionMetaDataByContentId())
                    .bind(0, contentId).map(ArtifactMetaDataDtoMapper.instance).list();
            if (dtos.isEmpty()) {
    ...
    public List<Long> getArtifactContentIds(String groupId, String artifactId) {
        return handles.withHandleNoException(handle -> {
            return handle.createQuery(sqlStatements().selectArtifactContentIds())
                    .bind(0, normalizeGroupId(groupId)).bind(1, artifactId).mapTo(Long.class).list();
        });
    }
     ...
     */
    private void updateArtifactVersionStateRaw(long globalId, ArtifactState oldState,
            ArtifactState newState) {
        handles.withHandleNoException(handle -> {
            artifactStateEx.applyState(s -> {
                handle.createUpdate(sqlStatements.updateArtifactVersionState()).bind(0, s.name())
     ...
     * IMPORTANT: Private methods can't be @Transactional. Callers MUST have started a transaction.
     */
    private ArtifactVersionMetaDataDto createArtifactVersionRaw(boolean firstVersion, String groupId,
            String artifactId, String version, String name, String description, List<String> labels,
        ...
        if (firstVersion) {
            ...
            handles.withHandleNoException(handle -> {
                ...
                handle.createUpdate(sqlStatements.insertVersion(true)).bind(0, globalId)
                        ...
                        .bind(4, state).bind(5, limitStr(name, 512))
                        .bind(6, limitStr(description, 1024, true)).bind(7, createdBy).bind(8, createdOn)
                        .bind(9, labelsStr).bind(10, propertiesStr).bind(11, contentId).execute();
            ...
            handles.withHandleNoException(handle -> {
                ...
                handle.createUpdate(sqlStatements.insertVersion(false)).bind(0, globalId)
                        .bind(1, normalizeGroupId(groupId)).bind(2, artifactId).bind(3, finalVersion2)
                        .bind(4, normalizeGroupId(groupId)).bind(5, artifactId).bind(6, state)
                        .bind(7, limitStr(name, 512)).bind(8, limitStr(description, 1024, true))
                        .bind(9, createdBy).bind(10, createdOn).bind(11, labelsStr).bind(12, propertiesStr)
                        .bind(13, contentId).execute();

                // If version is null, update the row we just inserted to set the version to the generated
                // versionId
                if (finalVersion2 == null) {

                    handle.createUpdate(sqlStatements.autoUpdateVersionForGlobalId()).bind(0, globalId)
                            .execute();
                }
        ...
        return handles.withHandleNoException(handle -> {
            ...
            if (labels != null && !labels.isEmpty()) {
                labels.forEach(label -> {
                    ...
                    handle.createUpdate(sqlStatements.insertLabel()).bind(0, globalId)
                            .bind(1, limitStr(label.toLowerCase(), 256)).execute();
                });
            }
            ...
            if (properties != null && !properties.isEmpty()) {
                properties.forEach((k, v) -> {
                    ...
                    handle.createUpdate(sqlStatements.insertProperty()).bind(0, globalId)
                            .bind(1, limitStr(k.toLowerCase(), 256)).bind(2, limitStr(v.toLowerCase(), 1024))
                            .execute();
                });
            }

            // Update the "latest" column in the artifacts table with the globalId of the new version
            handle.createUpdate(sqlStatements.updateArtifactLatest()).bind(0, globalId)
            ...
            return handle.createQuery(sqlStatements.selectArtifactVersionMetaDataByGlobalId())
                    .bind(0, globalId).map(ArtifactVersionMetaDataDtoMapper.instance).one();
     ...
     */
    private Long getOrCreateContent(String artifactType, ContentHandle content,
            List<ArtifactReferenceDto> references) {
        if (notEmpty(references)) {
            return getOrCreateContentRaw(content, utils.getContentHash(content, references),
            ...
            return getOrCreateContentRaw(content, utils.getContentHash(content, null),
     ...
     * IMPORTANT: Private methods can't be @Transactional. Callers MUST have started a transaction.
     */
    private Long getOrCreateContentRaw(ContentHandle content, String contentHash, String canonicalContentHash,
        ...
        return handles.withHandleNoException(handle -> {
            ...
            if (Set.of("mssql", "postgresql").contains(sqlStatements.dbType())) {
                ...
                handle.createUpdate(sqlStatements.upsertContent()).bind(0, nextContentId())
                        .bind(1, canonicalContentHash).bind(2, contentHash).bind(3, contentBytes)
                        .bind(4, referencesSerialized).execute();
                ...
                throw new UnsupportedOperationException(
                        "Unsupported database type: " + sqlStatements.dbType());
            }

            if (insertReferences) {
                // Finally, insert references into the "artifactreferences" table if the content wasn't
                // present yet.
    ...
    private void insertReferences(Long contentId, List<ArtifactReferenceDto> references) {
        if (references != null && !references.isEmpty()) {
            references.forEach(reference -> {
                handles.withHandleNoException(handle -> {
                    try {
                        handle.createUpdate(sqlStatements.upsertReference()).bind(0, contentId)
    ...
    public ArtifactMetaDataDto createArtifactWithMetadata(String groupId, String artifactId, String version,
        ...
        if (groupId != null && !isGroupExists(groupId)) {
            ...
            createGroup(GroupMetaDataDto.builder().groupId(groupId).createdOn(createdOn.getTime())
                    .modifiedOn(createdOn.getTime()).createdBy(createdBy).modifiedBy(createdBy).build());
        }
        ...
        return createArtifactWithMetadataRaw(groupId, artifactId, version, artifactType, contentId, createdBy,
                createdOn, md, null);
    }
     ...
     * IMPORTANT: Private methods can't be @Transactional. Callers MUST have started a transaction.
     */
    private ArtifactMetaDataDto createArtifactWithMetadataRaw(String groupId, String artifactId,
            String version, String artifactType, long contentId, String createdBy, Date createdOn,
        ...
        try {
            return handles.withHandle(handle -> {
                ...
                handle.createUpdate(sqlStatements.insertArtifact()).bind(0, normalizeGroupId(groupId))
                        .bind(1, artifactId).bind(2, artifactType).bind(3, createdBy).bind(4, createdOn)
                        .execute();

                // Then create a row in the content and versions tables (for the content and version
                // meta-data)
                ArtifactVersionMetaDataDto vmdd = createArtifactVersionRaw(true, groupId, artifactId, version,
                        metaData.getName(), metaData.getDescription(), metaData.getLabels(),
                        metaData.getProperties(), createdBy, createdOn, contentId, globalIdGenerator);
    ...
    public List<String> deleteArtifact(String groupId, String artifactId)
        ...
        try {
            List<String> res = handles.withHandle(handle -> {
                ...
                // Delete labels
                handle.createUpdate(sqlStatements.deleteLabels()).bind(0, normalizeGroupId(groupId))
                        .bind(1, artifactId).execute();

                // Delete properties
                handle.createUpdate(sqlStatements.deleteProperties()).bind(0, normalizeGroupId(groupId))
                        .bind(1, artifactId).execute();

                // Delete versions
                handle.createUpdate(sqlStatements.deleteVersions()).bind(0, normalizeGroupId(groupId))
                        .bind(1, artifactId).execute();
    ...
    public void deleteArtifacts(String groupId) throws RegistryStorageException {
        ...
        try {
            handles.withHandle(handle -> {
                ...
                // Delete labels
                handle.createUpdate(sqlStatements.deleteLabelsByGroupId()).bind(0, normalizeGroupId(groupId))
                        .execute();
    ...
    @Transactional
    public StoredArtifactDto getArtifact(String groupId, String artifactId,
            ArtifactRetrievalBehavior behavior) throws ArtifactNotFoundException, RegistryStorageException {
        log.debug("Selecting a single artifact (latest version) by artifactId: {} {} (behavior = {})",
                groupId, artifactId, behavior);
        try {
            switch (behavior) {
                case DEFAULT: {
                    return handles.withHandle(handle -> {
                        Optional<StoredArtifactDto> res = handle
                                .createQuery(sqlStatements.selectLatestArtifactContent())
                ...
                case SKIP_DISABLED_LATEST: {
                    return handles.withHandle(handle -> {
                        ...
                        Optional<StoredArtifactDto> res = handle
                                .createQuery(sqlStatements.selectLatestArtifactContentSkipDisabledState())
                                .bind(0, normalizeGroupId(groupId)).bind(1, artifactId)
                                .map(StoredArtifactMapper.instance).findOne();
                        if (res.isEmpty()) {
                            // Unlikely, but the latest artifact version may be disabled
                            res = handle
                                    .createQuery(sqlStatements
                                            .selectLatestArtifactContentWithMaxGlobalIDSkipDisabledState())
                                    .bind(0, normalizeGroupId(groupId)).bind(1, artifactId)
                                    // INNER:
                                    .bind(2, normalizeGroupId(groupId)).bind(3, artifactId)
                                    .map(StoredArtifactMapper.instance).findOne();
                        }
                        return res.orElseThrow(() -> new ArtifactNotFoundException(groupId, artifactId));
                    });
    ...
    public ArtifactMetaDataDto updateArtifactWithMetadata(String groupId, String artifactId, String version,
        ...
        return updateArtifactWithMetadata(groupId, artifactId, version, artifactType, content, metaData,
                references, null);
    }

    protected ArtifactMetaDataDto updateArtifactWithMetadata(String groupId, String artifactId,
            String version, String artifactType, ContentHandle content, EditableArtifactMetaDataDto metaData,
            List<ArtifactReferenceDto> references, IdGenerator globalIdGenerator)
            throws ArtifactNotFoundException, RegistryStorageException {
     ...
     * IMPORTANT: Private methods can't be @Transactional. Callers MUST have started a transaction.
     */
    private ArtifactMetaDataDto updateArtifactWithMetadataRaw(String groupId, String artifactId,
            String version, long contentId, String createdBy, Date createdOn,
            EditableArtifactMetaDataDto metaData, IdGenerator globalIdGenerator)
            throws ArtifactNotFoundException, RegistryStorageException {
        log.debug("Updating artifact {} {} with a new version (content).", groupId, artifactId);

        // For the update we want to get meta-data from previous (latest) existing version, no matter the
        // state.
        ...
        try {
            ...
            return handles.withHandle(handle -> {
                ...
                ArtifactVersionMetaDataDto versionDto = createArtifactVersionRaw(false, groupId, artifactId,
                        version, name, description, labels, properties, createdBy, createdOn, contentId,
                        globalIdGenerator);
        ...
        return handles.withHandleNoException(handle -> {
            ...
            select.append(
                    "SELECT a.*, v.globalId, v.version, v.state, v.name, v.description, v.labels, v.properties, "
                            + "v.createdBy AS modifiedBy, v.createdOn AS modifiedOn " + "FROM artifacts a "
            ...
            for (SearchFilter filter : filters) {
                ...
                switch (filter.getType()) {
                    case description:
                        ...
                        break;
                    case everything:
                        where.append("(" + "v.name LIKE ? OR " + "v.groupId LIKE ? OR "
                                + "a.artifactId LIKE ? OR " + "v.description LIKE ? OR "
                        ...
                        binders.add((query, idx) -> {
                            // Note: convert search to lowercase when searching for properties
                    ...
                    case properties:
                        ...
                        // Note: convert search to lowercase when searching for properties (case-insensitivity
                        // support).
            ...
            String artifactsQuerySql = select.toString() + where.toString() + orderByQuery.toString()
                    + limitOffset.toString();
            Query artifactsQuery = handle.createQuery(artifactsQuerySql);
            // Query for the total row count
            String countSelect = "SELECT count(a.artifactId) " + "FROM artifacts a "
                    + "JOIN versions v ON a.latest = v.globalId ";
            if (joinContentTable) {
    ...
    public ArtifactMetaDataDto getArtifactMetaData(String groupId, String artifactId,
            ArtifactRetrievalBehavior behavior) throws ArtifactNotFoundException, RegistryStorageException {
        log.debug("Selecting artifact (latest version) meta-data: {} {} (behavior = {})", groupId, artifactId,
                behavior);
        try {
            switch (behavior) {
                case DEFAULT: {
                    return handles.withHandle(handle -> {
                        Optional<ArtifactMetaDataDto> res = handle
                                .createQuery(sqlStatements.selectLatestArtifactMetaData())
                                .bind(0, normalizeGroupId(groupId)).bind(1, artifactId)
                ...
                case SKIP_DISABLED_LATEST: {
                    return handles.withHandle(handle -> {
                        // Try the likely option first using a more lightweight query
                        Optional<ArtifactMetaDataDto> res = handle
                                .createQuery(sqlStatements.selectLatestArtifactMetaDataSkipDisabledState())
                                .bind(0, normalizeGroupId(groupId)).bind(1, artifactId)
                                .map(ArtifactMetaDataDtoMapper.instance).findOne();
                        if (res.isEmpty()) {
                            // Unlikely, but the latest artifact version may be disabled
                            res = handle
                                    .createQuery(sqlStatements
                                            .selectLatestArtifactMetaDataWithMaxGlobalIDSkipDisabledState())
                                    .bind(0, normalizeGroupId(groupId)).bind(1, artifactId)
                                    // INNER:
                                    .bind(2, normalizeGroupId(groupId)).bind(3, artifactId)
                                    .map(ArtifactMetaDataDtoMapper.instance).findOne();
     ...
     * @param references may be null
     */
    private String getContentHash(String groupId, String artifactId, boolean canonical, ContentHandle content,
            List<ArtifactReferenceDto> references) {
        if (canonical) {
            var artifactMetaData = getArtifactMetaData(groupId, artifactId);
            return utils.getCanonicalContentHash(content, artifactMetaData.getType(), references,
    ...
    public ArtifactVersionMetaDataDto getArtifactVersionMetaData(String groupId, String artifactId,
            boolean canonical, ContentHandle content, List<ArtifactReferenceDto> references)
            throws ArtifactNotFoundException, RegistryStorageException {
    ...
    public ArtifactMetaDataDto getArtifactMetaData(long globalId)
        ...
        try {
            return handles.withHandle(handle -> {
                Optional<ArtifactMetaDataDto> res = handle
                        .createQuery(sqlStatements.selectArtifactMetaDataByGlobalId()).bind(0, globalId)
                        .map(ArtifactMetaDataDtoMapper.instance).findOne();
    ...
    public void updateArtifactMetaData(String groupId, String artifactId,
        ...
        var metadata = getArtifactMetaData(groupId, artifactId,
                storageBehaviorProps.getDefaultArtifactRetrievalBehavior());
        updateArtifactVersionMetadataRaw(metadata.getGlobalId(), groupId, artifactId, metadata.getVersion(),
                editableMetadata);
    }
        ...
        try {
            handles.withHandle(handle -> {
                int rowCount = handle.createUpdate(sqlStatements.updateArtifactOwner())
                        .bind(0, owner.getOwner()).bind(1, normalizeGroupId(groupId)).bind(2, artifactId)
                        .execute();
                if (rowCount == 0) {
    ...
    public RuleConfigurationDto getArtifactRule(String groupId, String artifactId, RuleType rule)
        ...
        try {
            return handles.withHandle(handle -> {
                Optional<RuleConfigurationDto> res = handle
                        .createQuery(sqlStatements.selectArtifactRuleByType())
        ...
        try {
            handles.withHandle(handle -> {
                int rowCount = handle.createUpdate(sqlStatements.updateArtifactRule())
                        ...
                        .bind(2, artifactId).bind(3, rule.name()).execute();
                if (rowCount == 0) {
                    if (!isArtifactExists(groupId, artifactId)) {
                        throw new ArtifactNotFoundException(groupId, artifactId);
                    }
            ...
            case DEFAULT:
                try {
                    return this.handles.withHandle(handle -> {
                        ...
                        List<String> versions = handle.createQuery(sql).bind(0, normalizeGroupId(groupId))
                                .bind(1, artifactId).mapTo(String.class).list();
                        if (versions.isEmpty()) {
                            throw new ArtifactNotFoundException(groupId, artifactId);
                        }
                        return versions;
    ...
    public VersionSearchResultsDto searchVersions(String groupId, String artifactId, int offset, int limit) {
        ...
        return handles.withHandleNoException(handle -> {
            ...
            Query query = handle.createQuery(sqlStatements.selectAllArtifactVersions())
                    .bind(0, normalizeGroupId(groupId)).bind(1, artifactId);
            if ("mssql".equals(sqlStatements.dbType())) {
                query.bind(2, offset).bind(3, limit);
            } else {
                query.bind(2, limit).bind(3, offset);
            }
    ...
    public void deleteArtifactVersion(String groupId, String artifactId, String version)
        ...
        // For deleting artifact versions we need to list always every single version, including disabled
        // ones.
        ...
        try {
            handles.withHandle(handle -> {
                ...
                handle.createUpdate(sqlStatements.updateArtifactLatest()).bind(0, (Long) null)
                        .bind(1, normalizeGroupId(groupId)).bind(2, artifactId).execute();

                // TODO use CASCADE when deleting rows from the "versions" table

                // Delete labels
                handle.createUpdate(sqlStatements.deleteVersionLabels()).bind(0, normalizeGroupId(groupId))
                ...
                // Delete comments
                handle.createUpdate(sqlStatements.deleteVersionComments()).bind(0, normalizeGroupId(groupId))
                        .bind(1, artifactId).bind(2, version).execute();
                ...
                // If the row was deleted, update the "latest" column to the globalId of the highest remaining
                // version
                if (rows == 1) {
                    versions.remove(version);

                    // Update the 'latest' version of the artifact to the globalId of the highest remaining
                    // version
                    ...
                    int latestUpdateRows = handle.createUpdate(sqlStatements.updateArtifactLatestGlobalId())
                            .bind(0, normalizeGroupId(groupId)).bind(1, artifactId).bind(2, latestVersion)
                            .bind(3, normalizeGroupId(groupId)).bind(4, artifactId).execute();
                ...
                if (rows > 1) {
                    throw new RegistryStorageException(
                            "Multiple versions deleted, artifact latest column left null");
                }
    ...
    public void updateArtifactVersionMetaData(String groupId, String artifactId, String version,
        ...
        var metadata = getArtifactVersionMetaData(groupId, artifactId, version);
        updateArtifactVersionMetadataRaw(metadata.getGlobalId(), groupId, artifactId, version,
                editableMetadata);
    }
     ...
     * IMPORTANT: Private methods can't be @Transactional. Callers MUST have started a transaction.
     */
    private void updateArtifactVersionMetadataRaw(long globalId, String groupId, String artifactId,
            String version, EditableArtifactMetaDataDto metaData) {
        try {
            handles.withHandle(handle -> {
                int rowCount = handle.createUpdate(sqlStatements.updateArtifactVersionMetaData())
                        ...
                        .bind(4, normalizeGroupId(groupId)).bind(5, artifactId).bind(6, version).execute();
                if (rowCount == 0) {
                    throw new VersionNotFoundException(groupId, artifactId, version);
                }

                // Delete all appropriate rows in the "labels" table
                handle.createUpdate(sqlStatements.deleteLabelsByGlobalId()).bind(0, globalId).execute();

                // Delete all appropriate rows in the "properties" table
                handle.createUpdate(sqlStatements.deletePropertiesByGlobalId()).bind(0, globalId).execute();
                ...
                if (labels != null && !labels.isEmpty()) {
                    labels.forEach(label -> {
                        ...
                        handle.createUpdate(sqli).bind(0, globalId)
                                .bind(1, limitStr(label.toLowerCase(), 256)).execute();
                    });
                }
                ...
                if (properties != null && !properties.isEmpty()) {
                    properties.forEach((k, v) -> {
                        ...
                        handle.createUpdate(sqli).bind(0, globalId).bind(1, limitStr(k.toLowerCase(), 256))
                                .bind(2, limitStr(v.toLowerCase(), 1024)).execute();
                    });
                }
    ...
    public void deleteArtifactVersionMetaData(String groupId, String artifactId, String version)
            throws ArtifactNotFoundException, VersionNotFoundException, RegistryStorageException {
        log.debug("Deleting user-defined meta-data for artifact {} {} version {}", groupId, artifactId,
                version);
        try {
            handles.withHandle(handle -> {
                // NULL out the name, description, labels, and properties columns of the "versions" table.
                int rowCount = handle.createUpdate(sqlStatements.updateArtifactVersionMetaData())
                        .bind(0, (String) null).bind(1, (String) null).bind(2, (String) null)
                        .bind(3, (String) null).bind(4, normalizeGroupId(groupId)).bind(5, artifactId)
                        .bind(6, version).execute();
    ...
    public CommentDto createArtifactVersionComment(String groupId, String artifactId, String version,
        ...
        return createArtifactVersionCommentRaw(groupId, artifactId, theVersion, this::nextCommentId,
    ...
    public CommentDto createArtifactVersionCommentRaw(String groupId, String artifactId, String version,
        ...
        try {
            return handles.withHandle(handle -> {
                ...
                var entity = CommentEntity.builder().commentId(String.valueOf(commentId.generate()))
                        .globalId(metadata.getGlobalId()).createdBy(createdBy).createdOn(createdOn.getTime())
                        .value(value).build();
                ...
                return CommentDto.builder().commentId(entity.commentId).createdBy(createdBy)
    ...
    public List<CommentDto> getArtifactVersionComments(String groupId, String artifactId, String version) {
        ...
        try {
            return handles.withHandle(handle -> {
                return handle.createQuery(sqlStatements.selectComments()).bind(0, normalizeGroupId(groupId))
                        .bind(1, artifactId).bind(2, theVersion).map(CommentDtoMapper.instance).list();
            });
    ...
    public void deleteArtifactVersionComment(String groupId, String artifactId, String version,
        ...
        try {
            handles.withHandle(handle -> {
                Optional<ArtifactVersionMetaDataDto> res = handle
                        .createQuery(sqlStatements.selectArtifactVersionMetaData())
                        .bind(0, normalizeGroupId(groupId)).bind(1, artifactId).bind(2, theVersion)
                        .map(ArtifactVersionMetaDataDtoMapper.instance).findOne();
                ArtifactVersionMetaDataDto avmdd = res
                        .orElseThrow(() -> new VersionNotFoundException(groupId, artifactId, version));

                int rowCount = handle.createUpdate(sqlStatements.deleteComment()).bind(0, avmdd.getGlobalId())
                        .bind(1, commentId).bind(2, deletedBy).execute();
                if (rowCount == 0) {
    ...
    public void updateArtifactVersionComment(String groupId, String artifactId, String version,
        ...
        try {
            handles.withHandle(handle -> {
                ...
                int rowCount = handle.createUpdate(sqlStatements.updateComment()).bind(0, value)
    ...
    public RuleConfigurationDto getGlobalRule(RuleType rule)
        ...
        try {
            return handles.withHandle(handle -> {
                Optional<RuleConfigurationDto> res = handle
                        .createQuery(sqlStatements.selectGlobalRuleByType()).bind(0, rule.name())
                        .map(RuleConfigurationDtoMapper.instance).findOne();
        ...
        try {
            return handles.withHandle(handle -> {
                ...
                Optional<DynamicConfigPropertyDto> res = handle
                        ...
                        .bind(0, normalizedPropertyName).map(DynamicConfigPropertyDtoMapper.instance)
                        .findOne();
        ...
        try {
            handles.withHandle(handle -> {
                handle.createUpdate(sqlStatements.insertGroup()).bind(0, group.getGroupId())
                        ...
                        .bind(7, SqlUtil.serializeProperties(group.getProperties())).execute();
                return null;
            });
        } catch (Exception e) {
            if (sqlStatements.isPrimaryKeyViolation(e)) {
    ...
    public void updateGroupMetaData(GroupMetaDataDto group)
        ...
        handles.withHandleNoException(handle -> {
            int rows = handle.createUpdate(sqlStatements.updateGroup()).bind(0, group.getDescription())
                    .bind(1, group.getArtifactsType()).bind(2, group.getModifiedBy())
                    .bind(3, group.getModifiedOn())
                    .bind(4, SqlUtil.serializeProperties(group.getProperties())).bind(5, group.getGroupId())
                    .execute();
            if (rows == 0) {
        ...
        try {
            ...
            handles.withHandle(handle -> {
                Stream<ContentEntity> stream = handle.createQuery(sqlStatements.exportContent())
                        .setFetchSize(50).map(ContentEntityMapper.instance).stream();
            ...
            handles.withHandle(handle -> {
                Stream<ArtifactVersionEntity> stream = handle
                        .createQuery(sqlStatements.exportArtifactVersions()).setFetchSize(50)
                        .map(ArtifactVersionEntityMapper.instance).stream();
                // Process and then close the stream.
                try (stream) {
                    stream.forEach(handler::apply);
                }
    ...
    public RoleMappingDto getRoleMapping(String principalId) throws RegistryStorageException {
        ...
        try {
            return handles.withHandle(handle -> {
                Optional<RoleMappingDto> res = handle
                        .createQuery(sqlStatements.selectRoleMappingByPrincipalId()).bind(0, principalId)
                        .map(RoleMappingDtoMapper.instance).findOne();
    ...
    public DownloadContextDto consumeDownload(String downloadId) throws RegistryStorageException {
        ...
        try {
            return handles.withHandle(handle -> {
                ...
                int rowCount = handle.createUpdate(sqlStatements.deleteDownload()).bind(0, downloadId)
                        .execute();
                if (rowCount == 0) {
    ...
    public void deleteAllUserData() {
        ...
        handles.withHandleNoException(handle -> {
            ...
            handle.createUpdate(sqlStatements.deleteAllReferences()).execute();

            handle.createUpdate(sqlStatements.deleteAllLabels()).execute();

            handle.createUpdate(sqlStatements.deleteAllProperties()).execute();

            handle.createUpdate(sqlStatements.deleteAllComments()).execute();
     ...
     * IMPORTANT: Private methods can't be @Transactional. Callers MUST have started a transaction.
     */
    private void resolveReferences(Map<String, ContentHandle> resolvedReferences,
            List<ArtifactReferenceDto> references) {
        if (references != null && !references.isEmpty()) {
                ...
                if (reference.getArtifactId() == null || reference.getName() == null
                    ...
                    if (!resolvedReferences.containsKey(reference.getName())) {
                        ...
                        try {
                            ...
                            final ContentWrapperDto referencedContent = getArtifactByContentId(
    ...
    private void deleteAllOrphanedContent() {
        ...
        handles.withHandleNoException(handle -> {
            ...
            handle.createUpdate(sqlStatements.deleteOrphanedReferences()).execute();
        ...
        handles.withHandleNoException(handle -> {
            ...
            if (!isArtifactExists(entity.groupId, entity.artifactId)) {
                ...
                handle.createUpdate(sqlStatements.insertArtifact()).bind(0, normalizeGroupId(entity.groupId))
                        .bind(1, entity.artifactId).bind(2, entity.artifactType).bind(3, entity.createdBy)
                        .bind(4, new Date(entity.createdOn)).execute();
            }
            ...
            if (!isGlobalIdExists(entity.globalId)) {
                ...
                handle.createUpdate(sqlStatements.importArtifactVersion()).bind(0, entity.globalId)
                        ...
                        .bind(3, entity.version).bind(4, entity.versionId).bind(5, entity.state)
                        .bind(6, entity.name).bind(7, entity.description).bind(8, entity.createdBy)
                        .bind(9, new Date(entity.createdOn)).bind(10, SqlUtil.serializeLabels(entity.labels))
                        .bind(11, SqlUtil.serializeProperties(entity.properties)).bind(12, entity.contentId)
                        .execute();

                // Insert labels into the "labels" table
                if (entity.labels != null && !entity.labels.isEmpty()) {
                    entity.labels.forEach(label -> {
                        handle.createUpdate(sqlStatements.insertLabel()).bind(0, entity.globalId)
                                .bind(1, label.toLowerCase()).execute();
                    });
                }
                ...
                if (entity.properties != null && !entity.properties.isEmpty()) {
                    entity.properties.forEach((k, v) -> {
                        handle.createUpdate(sqlStatements.insertProperty()).bind(0, entity.globalId)
                ...
                if (entity.isLatest) {
                    // Update the "latest" column in the artifacts table with the globalId of the new version
                    handle.createUpdate(sqlStatements.updateArtifactLatest()).bind(0, entity.globalId)
                            .bind(1, normalizeGroupId(entity.groupId)).bind(2, entity.artifactId).execute();
                }
        ...
        handles.withHandleNoException(handle -> {
            ...
            if (!isContentExists(entity.contentId)) {
                ...
                handle.createUpdate(sqlStatements.importContent()).bind(0, entity.contentId)
                        .bind(1, entity.canonicalHash).bind(2, entity.contentHash)
                        .bind(3, entity.contentBytes).bind(4, entity.serializedReferences).execute();

                insertReferences(entity.contentId,
        ...
        if (!isGroupExists(entity.groupId)) {
            handles.withHandleNoException(handle -> {
                handle.createUpdate(sqlStatements.importGroup())
                        ...
                        .bind(2, entity.artifactsType).bind(3, entity.createdBy)
                        .bind(4, new Date(entity.createdOn)).bind(5, entity.modifiedBy)
                        .bind(6, new Date(entity.modifiedOn))
                        .bind(7, SqlUtil.serializeProperties(entity.properties)).execute();
                return null;
            });
        } else {
    ...
    public void importComment(CommentEntity entity) {
        handles.withHandleNoException(handle -> {
            handle.createUpdate(sqlStatements.insertComment()).bind(0, entity.commentId)
                    .bind(1, entity.globalId).bind(2, entity.createdBy).bind(3, new Date(entity.createdOn))
    ...
    public String normalizeVersion(String groupId, String artifactId, String version) {
        if ("latest".equalsIgnoreCase(version)) {
            return getArtifactMetaData(groupId, artifactId, ArtifactRetrievalBehavior.SKIP_DISABLED_LATEST)
                    .getVersion();
        }
        return version;
    }
    ...
    private static boolean hasContentFilter(Set<SearchFilter> filters) {
        for (SearchFilter searchFilter : filters) {
            if (searchFilter.getType() == SearchFilterType.contentHash
                    || searchFilter.getType() == SearchFilterType.canonicalHash) {
                return true;
            }
        }
        return false;
