...
public class ArtifactTypeUtilProviderImpl implements ArtifactTypeUtilProviderFactory {
    ...
    @Inject
    Instance<ArtifactTypeUtilProvider> adapters;

    public ArtifactTypeUtilProvider getArtifactTypeAdapter(ArtifactType type) {
        return map.computeIfAbsent(type, t ->
            adapters.stream()
                    .filter(a -> a.getArtifactType() == t)
                    .findFirst()
                    .orElseThrow(() -> new IllegalStateException("No such artifact type adapter: " + t)));
    }
}
