...
CREATE TABLE webhook_subscriptions (subscriptionId VARCHAR(128) NOT NULL, endpointUrl VARCHAR(1024) NOT NULL, eventTypes TEXT, groupFilter VARCHAR(512), artifactFilter VARCHAR(512), authType VARCHAR(32) NOT NULL DEFAULT 'NONE', authConfig TEXT, isEnabled BOOLEAN NOT NULL DEFAULT TRUE, owner VARCHAR(256), createdOn TIMESTAMP WITHOUT TIME ZONE NOT NULL, modifiedBy VARCHAR(256), modifiedOn TIMESTAMP WITHOUT TIME ZONE);
ALTER TABLE webhook_subscriptions ADD PRIMARY KEY (subscriptionId);
CREATE HASH INDEX IDX_whsubs_1 ON webhook_subscriptions(isEnabled);
CREATE INDEX IDX_whsubs_2 ON webhook_subscriptions(createdOn);
CREATE TABLE webhook_delivery_logs (deliveryId BIGINT AUTO_INCREMENT NOT NULL, subscriptionId VARCHAR(128) NOT NULL, eventId VARCHAR(128) NOT NULL, attemptCount INT NOT NULL DEFAULT 0, lastAttemptOn TIMESTAMP WITHOUT TIME ZONE, status VARCHAR(32) NOT NULL, responseCode INT, createdOn TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP);
...
CREATE HASH INDEX IDX_whdlogs_3 ON webhook_delivery_logs(status);