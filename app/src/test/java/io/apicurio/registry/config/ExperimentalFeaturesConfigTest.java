 ...
 *
 * <p>The auto-discovered boolean property list is a Mockito mock; its
 * {@code getExperimentalConfigProperties()} returns an empty collection by default, so the
 * build-time discovery loop is a no-op and each test isolates a single gating path.</p>
 */
