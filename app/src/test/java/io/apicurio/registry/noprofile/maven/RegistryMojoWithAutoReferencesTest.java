...
public class RegistryMojoWithAutoReferencesTest extends RegistryMojoTestBase {
    ...
    private void validateStructure(String groupId, String artifactId, int expectedMainReferences,
        ...
        final ArtifactMetaData artifactWithReferences = clientV3.groups().byGroupId(groupId).artifacts()
        ...
        final String mainContent = new String(clientV3.groups().byGroupId(groupId).artifacts()
                .byArtifactId(artifactId).versions().byVersion(artifactWithReferences.getVersion()).get()
        ...
        final List<ArtifactReference> mainArtifactReferences = clientV3.ids().globalIds()
                .byGlobalId(artifactWithReferences.getGlobalId()).references().get().get(3, TimeUnit.SECONDS);
        ...
        Assertions.assertEquals(expectedTotalArtifacts, clientV3.groups().byGroupId(groupId).artifacts().get()
                .get(3, TimeUnit.SECONDS).getCount().intValue());
    }
    ...
    private void validateReferences(List<ArtifactReference> artifactReferences, Set<String> loadedContents)
        ...
        for (ArtifactReference artifactReference : artifactReferences) {
            String referenceContent = new String(clientV3.groups().byGroupId(artifactReference.getGroupId())
                    ...
                    .byVersion(artifactReference.getVersion()).get().get(3, TimeUnit.SECONDS).readAllBytes(),
                    StandardCharsets.UTF_8);
            ArtifactMetaData referenceMetadata = clientV3.groups().byGroupId(artifactReference.getGroupId())
                    .artifacts().byArtifactId(artifactReference.getArtifactId()).meta().get()
            ...
            List<ArtifactReference> nestedReferences = clientV3.ids().globalIds()
                    .byGlobalId(referenceMetadata.getGlobalId()).references().get().get(3, TimeUnit.SECONDS);
