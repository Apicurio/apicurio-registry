...
""";
    ...
    public void testCreateDraftArtifact() throws Exception {
        ...
        CreateArtifact createArtifact = TestUtils.clientCreateArtifact(artifactId, ArtifactType.OPENAPI, content, ContentTypes.APPLICATION_JSON);
        createArtifact.getFirstVersion().setIsDraft(true);
        createArtifact.getFirstVersion().setVersion("1.0.0");

        clientV3.groups().byGroupId(groupId).artifacts().post(createArtifact);

        VersionMetaData vmd = clientV3.groups().byGroupId(groupId).artifacts().byArtifactId(artifactId).versions().byVersionExpression("1.0.0").get();
    ...
    public void testCreateDraftArtifactVersion() throws Exception {
        ...
        VersionMetaData vmd = clientV3.groups().byGroupId(groupId).artifacts().byArtifactId(artifactId).versions().post(createVersion);

        Assertions.assertNotNull(vmd);
        Assertions.assertEquals(VersionState.DRAFT, vmd.getState());

        vmd = clientV3.groups().byGroupId(groupId).artifacts().byArtifactId(artifactId).versions().byVersionExpression("1.0.0").get();
        Assertions.assertNotNull(vmd);
        Assertions.assertEquals(VersionState.ENABLED, vmd.getState());
        vmd = clientV3.groups().byGroupId(groupId).artifacts().byArtifactId(artifactId).versions().byVersionExpression("1.0.1").get();
        Assertions.assertNotNull(vmd);
        Assertions.assertEquals(VersionState.DRAFT, vmd.getState());
    }
    ...
    public void testUpdateDraftContent() throws Exception {
        ...
        CreateArtifact createArtifact = TestUtils.clientCreateArtifact(artifactId, ArtifactType.ASYNCAPI, AVRO_CONTENT_V1, ContentTypes.APPLICATION_JSON);
        createArtifact.getFirstVersion().setIsDraft(true);
        ...
        try (InputStream inputStream = clientV3.groups().byGroupId(groupId).artifacts().byArtifactId(artifactId).versions().byVersionExpression("1.0").content().get()) {
            String content = IOUtils.toString(inputStream, StandardCharsets.UTF_8);
            Assertions.assertEquals(TestUtils.normalizeMultiLineString(AVRO_CONTENT_V1), TestUtils.normalizeMultiLineString(content));
        }

        VersionContent versionContent = new VersionContent();
        versionContent.setContentType(ContentTypes.APPLICATION_JSON);
        versionContent.setContent(AVRO_CONTENT_V2);
        clientV3.groups().byGroupId(groupId).artifacts().byArtifactId(artifactId).versions().byVersionExpression("1.0").content().put(versionContent);
    ...
    public void testCannotUpdateNonDraftContent() throws Exception {
        ...
        Assertions.assertEquals("Requested artifact version is not in DRAFT state.  Update disallowed.", error.getTitle());
