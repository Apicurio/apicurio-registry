 ...
 *
 * <p>The test profile also disables {@code quarkus.http.enable-compression}. That Vert.x-layer
 * setting is normally a no-op for classic RESTEasy responses, but it can intermittently compress
 * the response after the interceptor has already been disabled, depending on flushing/chunking
 * timing (see issue #9245). Turning it off here isolates this test to the single layer it's
 * actually meant to verify: the {@code apicurio.rest.compression.enabled} kill switch.
 */
@QuarkusTest
...
public class HttpCompressionDisabledTest extends AbstractResourceTestBase {
    ...
    public static class CompressionDisabledProfile implements QuarkusTestProfile {
        ...
        public Map<String, String> getConfigOverrides() {
            return Map.of(
                    "apicurio.rest.compression.enabled", "false",
