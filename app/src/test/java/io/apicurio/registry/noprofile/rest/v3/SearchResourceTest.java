...
public class SearchResourceTest extends AbstractResourceTestBase {
    ...
    public void testSearchByGroup() throws Exception {
        ...
        given().when().queryParam("group", group).get("/registry/v3/search/artifacts").then().statusCode(200)
                .body("count", equalTo(5)).body("artifacts[0].groupId", equalTo(group));
    }
    ...
    public void testSearchByName() throws Exception {
        ...
        for (int idx = 0; idx < 2; idx++) {
            String artifactId = "Empty-" + idx;
            this.createArtifact(group, artifactId, ArtifactType.OPENAPI,
                    artifactContent.replaceAll("Empty API", name));
        }
    ...
    public void testSearchByLabels() throws Exception {
        ...
        TestUtils.retry(() -> {
            given().when().queryParam("labels", "testSearchByLabels").get("/registry/v3/search/artifacts")
                    .then().statusCode(200).body("count", equalTo(5));
        });

        TestUtils.retry(() -> {
            given().when().queryParam("labels", "testSearchByLabels-2").get("/registry/v3/search/artifacts")
    ...
    public void testSearchByProperties() throws Exception {
        ...
        TestUtils.retry(() -> {
            given().when().queryParam("properties", "all-key:all-value").get("/registry/v3/search/artifacts")
                    .then().statusCode(200).body("count", equalTo(5));
        });

        TestUtils.retry(() -> {
            given().when().queryParam("properties", "key-1:value-1").get("/registry/v3/search/artifacts")
                    .then().statusCode(200).body("count", equalTo(1));
        });

        TestUtils.retry(() -> {
            given().when().queryParam("properties", "key-1:value-1")
                    .queryParam("properties", "another-key-1:another-value-1")
                    .get("/registry/v3/search/artifacts").then().statusCode(200).body("count", equalTo(1));
        });

        // negative test cases
        TestUtils.retry(() -> {
            given().when().queryParam("properties", "key-1:value-1").queryParam("properties", "key-2:value-2")
                    .get("/registry/v3/search/artifacts").then().statusCode(200).body("count", equalTo(0));
        });
        TestUtils.retry(() -> {
            given().when().queryParam("properties", "key-1:value-1:").get("/registry/v3/search/artifacts")
                    .then().statusCode(200).body("count", equalTo(0));
        });
        TestUtils.retry(() -> {
            given().when().queryParam("properties", "key-1:").get("/registry/v3/search/artifacts").then()
                    .statusCode(400);
        });
        TestUtils.retry(() -> {
            given().when().queryParam("properties", ":value-1").get("/registry/v3/search/artifacts").then()
                    .statusCode(400);
        });
    }
    ...
    public void testSearchByPropertyKey() throws Exception {
        ...
        TestUtils.retry(() -> {
            given().when().queryParam("properties", "all-key").get("/registry/v3/search/artifacts").then()
                    .statusCode(200).body("count", equalTo(5));
        });
        TestUtils.retry(() -> {
            given().when().queryParam("properties", "a-key-1").get("/registry/v3/search/artifacts").then()
                    .statusCode(200).body("count", equalTo(1));
        });
        TestUtils.retry(() -> {
            given().when().queryParam("properties", "extra-key-0").get("/registry/v3/search/artifacts").then()
                    .statusCode(200).body("count", equalTo(3));
        });
        TestUtils.retry(() -> {
            given().when().queryParam("properties", "extra-key-2").get("/registry/v3/search/artifacts").then()
    ...
    public void testOrderBy() throws Exception {
        ...
        given().when().queryParam("orderby", "name").queryParam("order", "asc").queryParam("group", group)
                .get("/registry/v3/search/artifacts").then().statusCode(200).body("count", equalTo(5))
                .body("artifacts[0].name", equalTo("empty-0"));

        given().when().queryParam("orderby", "name").queryParam("order", "desc").queryParam("group", group)
                .get("/registry/v3/search/artifacts").then().statusCode(200).body("count", equalTo(5))
                .body("artifacts[0].name", equalTo("empty-4"));

        given().when().queryParam("orderby", "createdOn").queryParam("order", "asc")
                .queryParam("group", group).get("/registry/v3/search/artifacts").then().statusCode(200)
    ...
    public void testLimitAndOffset() throws Exception {
        ...
        given().when().queryParam("orderby", "createdOn").queryParam("order", "asc").queryParam("limit", 5)
                .queryParam("offset", 10).queryParam("group", group).get("/registry/v3/search/artifacts")
