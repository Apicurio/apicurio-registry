...
public class ProtobufSerdeTest extends AbstractResourceTestBase {
    ...
    // io.apicurio.registry.rest.client.exception.ArtifactAlreadyExistsException: An artifact with ID
    // 'google/protobuf/timestamp.proto' in group 'default' already exists.
    ...
    //
    // System.out.println("artifaaact " +
    // clientV2.listArtifactsInGroup(groupId).getArtifacts().get(0).getId());
    //
    ...
    public void testProto() throws Exception {
        try (Serializer<TestCmmn.UUID> serializer = new ProtobufKafkaSerializer<>(restClient);
            ...
            waitForSchema(globalId -> {
                try {
                    if (restClient.ids().globalIds().byGlobalId(globalId).get().get()
                            .readAllBytes().length > 0) {
                        ArtifactMetaData artifactMetadata = restClient.groups().byGroupId(groupId).artifacts()
                                .byArtifactId(topic).meta().get().get();
