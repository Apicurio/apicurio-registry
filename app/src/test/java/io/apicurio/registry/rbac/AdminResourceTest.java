...
public class AdminResourceTest extends AbstractResourceTestBase {
    ...
    public void testGlobalRules() throws Exception {
        ...
        TestUtils.retry(() -> {
            given().when().get("/registry/v3/admin/rules/VALIDITY").then().statusCode(200)
                    .contentType(ContentType.JSON).body("type", equalTo("VALIDITY"))
                    .body("config", equalTo("FULL"));
        });

        // Try to add the rule again - should get a 409
        TestUtils.retry(() -> {
            given().when().contentType(CT_JSON).body(rule).post("/registry/v3/admin/rules").then()
    ...
    public void testIntegrityRule() throws Exception {
        ...
        TestUtils.retry(() -> {
            given().when().get("/registry/v3/admin/rules/INTEGRITY").then().statusCode(200)
                    .contentType(ContentType.JSON).body("type", equalTo("INTEGRITY"))
    ...
    void testExportForBrowser() throws Exception {
        ...
        for (int idx = 0; idx < 5; idx++) {
            ...
            List<ArtifactReference> refs = idx > 0
                    ? getSingletonRefList(group, "Empty-" + (idx - 1), "1", "ref")
    ...
    void testImport() throws Exception {
        ...
        List<Comment> comments = clientV3.groups().byGroupId("ImportTest").artifacts()
                .byArtifactId("Artifact-1").versions().byVersion("1.0.2").comments().get()
                .get(3, TimeUnit.SECONDS);
        ...
        comments = clientV3.groups().byGroupId("ImportTest").artifacts().byArtifactId("Artifact-2").versions()
                .byVersion("1.0.1").comments().get().get(3, TimeUnit.SECONDS);
        assertNotNull(comments);
        ...
        var rule = clientV3.groups().byGroupId("ImportTest").artifacts().byArtifactId("Artifact-1").rules()
                .byRule(RuleType.VALIDITY.getValue()).get().get(3, TimeUnit.SECONDS);
        assertNotNull(rule);
        ...
        var lastArtifactMeta = clientV3.groups().byGroupId("ImportTest").artifacts()
                .byArtifactId("Artifact-3").meta().get().get(3, TimeUnit.SECONDS);
        ...
        var executionException = Assertions.assertThrows(ExecutionException.class,
                () -> clientV3.ids().globalIds().byGlobalId(1006L).get().get(3, TimeUnit.SECONDS));
    ...
    public void testRoleMappings() throws Exception {
        ...
        TestUtils.retry(() -> {
            given().when().get("/registry/v3/admin/roleMappings").then().statusCode(200)
                    .contentType(ContentType.JSON).body("[0].principalId", equalTo("TestUser"))
                    .body("[0].principalName", equalTo("Foo bar")).body("[0].role", equalTo("DEVELOPER"));
        });

        // Try to add the rule again - should get a 409
        TestUtils.retry(() -> {
            given().when().contentType(CT_JSON).body(mapping).post("/registry/v3/admin/roleMappings").then()
                    .statusCode(409).body("error_code", equalTo(409)).body("message", equalTo(
                            "A mapping for principal 'TestUser' and role 'DEVELOPER' already exists."));
        });
