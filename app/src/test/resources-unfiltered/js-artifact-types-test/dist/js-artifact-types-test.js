// ../../../../target/classes/META-INF/quickjs4j/ArtifactTypeScriptProvider_Builtins.mjs
function info(args0) {
  ArtifactTypeScriptProvider_Builtins.info(args0);
}
...
// dist/src/test/resources-unfiltered/js-artifact-types-test/RamlArtifactType.js
function acceptsContent(request) {
...
function testCompatibility(request) {
  const existingArtifacts = request.existingArtifacts;
  const proposedArtifact = request.proposedArtifact;
  if (existingArtifacts == null || existingArtifacts == void 0 || existingArtifacts.length === 0) {
    return {
...
function canonicalize(request) {
  ...
  const convertedContent = originalContent.replace("use to query all orders of a user", "USE TO QUERY ALL ORDERS OF A USER").replace("Lists all orders of a specific user", "LIST ALL ORDERS OF A SPECIFIC USER");
  return {
    typedContent: {
  ...
  if (node !== null && node !== void 0) {
    if (typeof node === "object") {
      ...
      array.forEach((childNode) => {
        if (childNode != null && childNode !== void 0) {
...
function rewriteReferences(request) {
  const root = parse(request.content.content);
...
function findExternalReferences(request) {
  request;
  return {};
}
export {
