...
public class GrantsData {
    ...
    public SearchFilterData getSearchFilterData(String user, Set<String> roles, String resourceType,
        ...
        Set<String> deniedPrefixResources = new HashSet<>();
        ...
        for (Grant grant : grants) {
            ...
            if (grant.deny()) {
                if (groupSeparator != null && grant.resourcePattern().contains(groupSeparator)) {
                    if ("exact".equals(grant.resourcePatternType())) {
                        ...
                        deniedPrefixResources.add(grant.resourcePattern());
                    }
                }
                continue;
            }
        ...
        if (hasWildcard) {
            if (deniedExactResources.isEmpty() && deniedPrefixResources.isEmpty()) {
                return SearchFilterData.all();
            }
            return new SearchFilterData(Set.of(), Set.of(), deniedExactResources,
                    deniedPrefixResources, true);
        }
        if (groups.isEmpty() && exactResources.isEmpty()) {
            return SearchFilterData.none();
        }
        return new SearchFilterData(groups, exactResources, deniedExactResources,
                deniedPrefixResources, false);
    }
