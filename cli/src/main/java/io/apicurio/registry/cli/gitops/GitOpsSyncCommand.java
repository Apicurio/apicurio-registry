...
public class GitOpsSyncCommand extends AbstractCommand {
    ...
    public void run(final OutputBuffer output) throws Exception {
        // Capture baseline timestamp before triggering sync so we can detect completion
        final OffsetDateTime baselineAttempt = wait
                ? client.getRegistryClient().admin().gitops().status().get().getLastSyncAttempt()
                : null;
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            final var currentAttempt = status.getLastSyncAttempt();

            final boolean syncCompleted = currentAttempt != null
                    && (baselineAttempt == null || currentAttempt.isAfter(baselineAttempt));

            if (syncCompleted && GitOpsUtil.SYNC_STATE_IDLE.equals(syncState)) {
                GitOpsUtil.printStatus(output, status, outputType);
                return;
            }
            if (syncCompleted && GitOpsUtil.SYNC_STATE_ERROR.equals(syncState)) {
                GitOpsUtil.printStatus(output, status, outputType);
