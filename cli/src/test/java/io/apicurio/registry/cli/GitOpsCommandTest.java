...
public class GitOpsCommandTest {
    ...
    private static final String STATUS_BASELINE_BODY = """
            {
              "syncState": "IDLE",
              "lastSuccessfulSync": "2026-08-31T09:50:00Z",
              "lastSyncAttempt": "2026-08-31T10:00:00Z",
              "groupCount": 5,
              "artifactCount": 12,
              "versionCount": 28,
              "errors": [],
    ...
    void syncWaitSucceedsWhenIdle() {
        stubSyncWaitBaseline("sync-wait");

        wireMock.stubFor(get(urlEqualTo(STATUS_PATH))
                .inScenario("sync-wait")
                .whenScenarioStateIs("baseline-fetched")
                .willReturn(aResponse()
                        .withStatus(200)
                        .withHeader("Content-Type", "application/json")
                        .withBody(STATUS_LOADING_BODY))
                .willSetStateTo("loading"));

        wireMock.stubFor(get(urlEqualTo(STATUS_PATH))
                .inScenario("sync-wait")
                .whenScenarioStateIs("loading")
                .willReturn(aResponse()
                        .withStatus(200)
                        .withHeader("Content-Type", "application/json")
                        .withBody(STATUS_BODY)));
    ...
    @Test
    void syncWaitSucceedsOnImmediateIdle() {
        stubSyncWaitBaseline("sync-immediate-idle");

        wireMock.stubFor(get(urlEqualTo(STATUS_PATH))
                .inScenario("sync-immediate-idle")
                .whenScenarioStateIs("baseline-fetched")
                .willReturn(aResponse()
                        .withStatus(200)
                        .withHeader("Content-Type", "application/json")
    ...
    @Test
    void syncWaitSkipsStaleIdleBeforeTimestampAdvances() {
        stubSyncWaitBaseline("sync-stale-idle");

        wireMock.stubFor(get(urlEqualTo(STATUS_PATH))
                .inScenario("sync-stale-idle")
                .whenScenarioStateIs("baseline-fetched")
                .willReturn(aResponse()
                        .withStatus(200)
                        .withHeader("Content-Type", "application/json")
                        .withBody(STATUS_BASELINE_BODY))
                .willSetStateTo("stale-observed"));

        wireMock.stubFor(get(urlEqualTo(STATUS_PATH))
                .inScenario("sync-stale-idle")
                .whenScenarioStateIs("stale-observed")
                .willReturn(aResponse()
                        .withStatus(200)
                        .withHeader("Content-Type", "application/json")
                        .withBody(STATUS_BODY)));
    ...
    void syncWaitFailsOnError() {
        stubSyncWaitBaseline("sync-error");

        wireMock.stubFor(get(urlEqualTo(STATUS_PATH))
                .inScenario("sync-error")
                .whenScenarioStateIs("baseline-fetched")
                .willReturn(aResponse()
                        .withStatus(200)
                        .withHeader("Content-Type", "application/json")
    ...
    void syncWaitTimesOut() {
        stubSyncWaitBaseline("sync-timeout");

        wireMock.stubFor(get(urlEqualTo(STATUS_PATH))
                .inScenario("sync-timeout")
                .whenScenarioStateIs("baseline-fetched")
                .willReturn(aResponse()
                        .withStatus(200)
                        .withHeader("Content-Type", "application/json")
    ...
    private void stubSyncWaitBaseline(final String scenario) {
        wireMock.stubFor(post(urlEqualTo(SYNC_PATH))
                .willReturn(aResponse().withStatus(204)));
        ...
        wireMock.stubFor(get(urlEqualTo(STATUS_PATH))
                ...
                .willReturn(aResponse()
                        ...
                        .withHeader("Content-Type", "application/json")
                        .withBody(STATUS_BASELINE_BODY))
                .willSetStateTo("baseline-fetched"));
    }
