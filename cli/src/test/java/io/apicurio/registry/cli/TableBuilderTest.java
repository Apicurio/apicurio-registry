...
public class TableBuilderTest {
    ...
    @Test
    public void testSingleColumnGetsTheFullWidthWhenShrinking() {
        var output = print(new TableBuilder()
                .setMaxWidth(20)
                .addColumns("VALUE")
                .addRow("v".repeat(50)));

        // With no separators to subtract, the only column is allocated the entire width.
        var separatorLine = output.lines().skip(1).findFirst().orElseThrow();
        assertThat(separatorLine).isEqualTo("-".repeat(20));
        // The content wraps into lines of exactly that width, with nothing lost.
        assertThat(output).contains("v".repeat(20));
        assertThat(output).doesNotContain("v".repeat(21));
        assertThat(output.chars().filter(c -> c == 'v').count()).isEqualTo(50);
    }

    @Test
    public void testAllColumnsShrinkUniformlyWhenEachExceedsFairShare() {
        var output = print(new TableBuilder()
                .setMaxWidth(33)
                .addColumns("ONE", "TWO", "THREE")
                .addRow("a".repeat(50), "b".repeat(50), "c".repeat(50)));

        // 33 - 2 separators of 3 leaves 27, a fair share of 9 that every column exceeds,
        // so all of them shrink to it evenly.
        var separatorLine = output.lines().skip(1).findFirst().orElseThrow();
        assertThat(separatorLine).isEqualTo("-".repeat(9) + "   " + "-".repeat(9) + "   " + "-".repeat(9));
        // Every cell wraps within its shrunken column, with nothing lost.
        assertThat(output.chars().filter(c -> c == 'a').count()).isEqualTo(50);
        assertThat(output.chars().filter(c -> c == 'b').count()).isEqualTo(50);
        assertThat(output.chars().filter(c -> c == 'c').count()).isEqualTo(50);
    }
