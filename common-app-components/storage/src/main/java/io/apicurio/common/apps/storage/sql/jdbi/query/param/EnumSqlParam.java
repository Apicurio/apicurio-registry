 ...
 */
public class EnumSqlParam extends SqlParam<Enum<?>> {

    public EnumSqlParam(int position, Enum<?> value) {
    ...
    public void bindTo(PreparedStatement statement) {
        try {
            if (value == null) {
                statement.setNull(position + 1, Types.VARCHAR);
            } else {
                statement.setString(position + 1, value.name());
            }
