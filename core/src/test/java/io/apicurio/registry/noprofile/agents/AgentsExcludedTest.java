...
/**
 * Core's tests never have the agents module on their classpath, so this is what the server looks like
 * without the agent registry feature. Every agent flag is switched on, so any agent behaviour observed
 * here would mean core still carries agent code.
 */
@QuarkusTest
