// Metadata created by nebel

[id="installing-the-registry"]
= Installing {registry}

ifdef::apicurio-registry[]
This chapter explains how to install and run {registry} with the following storage options:

.Docker
* xref:installing-registry-in-memory-storage[]
* xref:installing-registry-jpa-storage[]
* xref:installing-registry-kafka-storage[]
* xref:installing-registry-kafka-streams-storage[]

.Kubernetes
* xref:setting-up-kafka-kubernetes-storage[]
* xref:installing-registry-kafka-kubernetes-storage[]
endif::[]
 
ifdef::rh-service-registry[]
This chapter explains how to set up storage in AMQ Streams and how to install and run {registry}:

* xref:setting-up-kafka-kubernetes-storage[]
* xref:installing-registry-kafka-kubernetes-storage[]
endif::[]
 
.Prerequisites
* xref:registry-overview[]

ifdef::apicurio-registry[]
.Additional resources
* For details on building from source, see https://github.com/Apicurio/apicurio-registry.
endif::[]

NOTE: You can install more than one instance of {registry} depending on your environment. The number of instances depends on your storage, for example, Kafka cluster configuration, and on the number and type of artifacts stored in the registry.

//INCLUDES
ifdef::apicurio-registry[]
include::{mod-loc}getting-started/proc_installing-registry-in-memory-storage.adoc[leveloffset=+1]
include::{mod-loc}getting-started/proc_installing-registry-jpa-storage.adoc[leveloffset=+1]
include::{mod-loc}getting-started/proc_installing-registry-kafka-storage.adoc[leveloffset=+1]
include::{mod-loc}getting-started/proc_installing-registry-kafka-streams-storage.adoc[leveloffset=+1]
include::{mod-loc}getting-started/proc_setting-up-kafka-kubernetes-storage.adoc[leveloffset=+1]
include::{mod-loc}getting-started/proc_installing-registry-kafka-kubernetes-storage.adoc[leveloffset=+1]
endif::[]

ifdef::rh-service-registry[]
include::{mod-loc}getting-started/proc_setting-up-kafka-kubernetes-storage.adoc[leveloffset=+1]
include::{mod-loc}getting-started/proc_installing-registry-kafka-kubernetes-storage.adoc[leveloffset=+1]
endif::[]
