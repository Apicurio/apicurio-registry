* Introduction
** xref:getting-started/assembly-intro-to-the-registry.adoc[]
** xref:getting-started/assembly-registry-concepts-glossary.adoc[]
** xref:getting-started/assembly-intro-to-registry-rules.adoc[]
** xref:getting-started/assembly-registry-compatibility-modes.adoc[]
** xref:getting-started/assembly-data-contracts.adoc[]
* Installation
** xref:getting-started/assembly-installing-registry-docker.adoc[]
** xref:getting-started/assembly-installing-registry-openshift.adoc[]
** xref:getting-started/assembly-installing-registry-storage-openshift.adoc[]
** xref:getting-started/assembly-configuring-kubernetesops-storage.adoc[]
** xref:getting-started/assembly-configuring-gitops-storage.adoc[]
** xref:getting-started/assembly-deploying-registry-operator.adoc[]
** xref:getting-started/assembly-deploying-registry-secured-kafka.adoc[]
** xref:getting-started/assembly-implementing-multitenancy.adoc[]
** xref:getting-started/assembly-registry-high-availability.adoc[]
** xref:getting-started/assembly-registry-sizing-guide.adoc[]
* Versioning and support
** xref:getting-started/assembly-versioning-support-policy.adoc[]
* Migration
** xref:getting-started/assembly-migrating-registry-v2-v3.adoc[]
* Configuration
** xref:getting-started/assembly-configuring-registry-security.adoc[]
** xref:getting-started/assembly-configuring-the-registry.adoc[]
ifndef::service-registry-downstream[]
** xref:getting-started/assembly-configuring-content-search.adoc[]
endif::[]
** xref:getting-started/assembly-custom-artifact-types.adoc[]
** xref:getting-started/assembly-registry-events.adoc[]
* Client applications
** xref:getting-started/assembly-managing-registry-artifacts-ui.adoc[]
** xref:getting-started/assembly-managing-registry-artifacts-api.adoc[]
** xref:getting-started/assembly-managing-registry-artifacts-cli.adoc[]
** xref:getting-started/assembly-using-the-registry-client.adoc[]
** xref:getting-started/assembly-managing-registry-artifacts-maven.adoc[]
* Apache Flink integration
** xref:getting-started/assembly-using-flink-catalog.adoc[]
* Apache Iceberg integration
** xref:getting-started/assembly-using-iceberg-catalog.adoc[]
* Kafka client serializers/deserialisers
** xref:getting-started/assembly-using-kafka-client-serdes.adoc[]
** xref:getting-started/assembly-configuring-kafka-client-serdes.adoc[]
* Usage Telemetry and Lifecycle Governance
** xref:getting-started/assembly-usage-telemetry.adoc[]
ifndef::service-registry-downstream[]
* Troubleshooting
** xref:getting-started/guide-exporting-registry-kafka-topic-data.adoc[]
endif::[]
* AI Agent Integration
** xref:getting-started/assembly-agent-registry-quickstart.adoc[]
** xref:getting-started/assembly-a2a-features.adoc[]
** xref:getting-started/assembly-mcp-tool-features.adoc[]
** xref:getting-started/assembly-mcp-server-integration.adoc[]
** xref:getting-started/assembly-agent-discovery-patterns.adoc[]
** xref:getting-started/assembly-ai-catalog-ard.adoc[]
** xref:getting-started/assembly-llm-artifact-types-implementation.adoc[]
* Confluent Schema Registry Compatibility
** xref:getting-started/assembly-confluent-schema-registry-compatibility.adoc[]
* Best Practices Guide
** xref:getting-started/assembly-schema-lifecycle-best-practices.adoc[]
* Reference
** xref:getting-started/assembly-artifact-reference.adoc[]
** xref:getting-started/assembly-rule-reference.adoc[]
** xref:getting-started/assembly-config-reference.adoc[]
** xref:getting-started/assembly-operator-config-reference.adoc[]
