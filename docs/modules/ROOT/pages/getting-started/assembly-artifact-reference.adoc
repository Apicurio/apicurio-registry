...
|`2.0.0`
|`MCP_SERVER`
|MCP server definitions (`server.json`, Model Context Protocol)
|`v0.1`
|`MCP_TOOL`
...
`MCP_SERVER`:: MCP (Model Context Protocol) server definitions, in the `server.json` format published to
an MCP registry. Use this artifact type to:
+
* Store the packages, remotes, and transports through which an MCP server can be reached
* Track server releases with version history
* Serve the servers through the MCP Registry API, which is enabled with `apicurio.mcp-registry.enabled`
* Validate server definitions against the MCP registry specification
+
