...
IMPORTANT: GitOps storage is an *experimental* feature. You must enable the experimental features gate (`APICURIO_FEATURES_EXPERIMENTAL_ENABLED=true`) to use it. If the gate is not enabled, the application fails at startup. The data format (`*-v0`) might change in future releases.
...
| `group-v0`
| An artifact group. Fields: `registryIds` (list), `groupId`, `description`, and `createdOn`.
...
The following example declares a registry configuration and an artifact:
...
Content files are plain schema files referenced by relative path. The registry detects the artifact content type from the file extension:
...
* The optional `validatedUpTo` field on an artifact controls rule validation during a load. When unset, only the latest version is validated against its predecessor. When set to a version identifier, versions up to and including that version are not re-validated.
...
----
. Mount the local clone of the repository into the registry container at `/repos/default`. A read-only mount is sufficient, because the registry does not write to the repository. The mount path is the workspace directory (`apicurio.gitops.workspace`, default `/repos`) combined with the repository directory name (`apicurio.gitops.repo.dir`, default `default`).
+
...
----
+
The registry reports not ready on its readiness probe until the first load succeeds. After the first successful load, it stays ready even if a later load fails, and it keeps serving the last successfully loaded data.
...
----
. Optional: Adjust the security level with the `APICURIO_GITOPS_SECURITY` environment variable. The default level, `strict`, requires a known hosts file for SSH pulls and rejects loose key file permissions. The `dev` level relaxes host key checking for development. Plaintext `http://` remote URLs are rejected at both levels.
...
----
+
The `repoId` value must match a configured repository. The response contains a `taskId`.
...
| `apicurio.gitops.validate.fetch.timeout.seconds` (`APICURIO_GITOPS_VALIDATE_FETCH_TIMEOUT_SECONDS`)
| `120`
| Timeout for the sync container to fetch a Git reference during validation.
...
| `lastSyncAttempt`
| Timestamp of the last load attempt.
