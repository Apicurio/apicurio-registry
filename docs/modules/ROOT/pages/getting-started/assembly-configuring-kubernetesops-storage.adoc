...
[role="_abstract"]
Configure {registry} to use KubernetesOps storage, including the required environment variables and an example deployment.
...
. Create the required RBAC resources as described in the RBAC requirements reference.

. Create ConfigMaps containing your registry data as described in the ConfigMap data format reference.
...
|`registry-v0`
|Registry configuration including global rules, settings, and peer registries
...
|`peers`
|list of peers
|Peer registries for federated search. Each peer has `peerId` (required, must be unique and cannot be `local`, which is reserved), `url` (required), optional `name` and `description`, `enabled` (boolean, defaults to `true` when omitted), and an optional `credentialSecretRef` (a reference to a credential, never a secret value). The full list is replaced on every load: omitting the field, or setting it to an empty list, removes all previously loaded peers.
|===
