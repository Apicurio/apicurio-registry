...
|Group-based access
|Not available
|Requires the `groups` claim in the OIDC token, which non-compliant providers do not issue.
|===
...
----
+
IMPORTANT: Both `appClientId` and `uiClientId` must be set to the public Dex client (`apicurio-registry-ui`). The `redirectUri` must be specified for silent token refresh to work. The `REGISTRY_API_URL` in the UI env section must use `https://` because the operator defaults to `http://`, which breaks when authentication is enabled.
+
...
* *CORS errors fetching `/.well-known/openid-configuration`*: Add `allowedOrigins` to the Dex `web` config with your {registry} UI route URL. Without this, the browser blocks the UI from discovering Dex endpoints.
...
* *`InvalidJwtSignatureException` or audience mismatch*: Both `appClientId` and `uiClientId` in the {registry} CR must match the Dex static client ID that the UI uses (`apicurio-registry-ui`). If they do not match, the backend will reject tokens issued for a different audience.
* *Groups not appearing in the token*: Verify that `QUARKUS_OIDC_AUTHENTICATION_SCOPES` includes `groups` on the backend, and `APICURIO_UI_AUTH_OIDC_SCOPE` includes `groups` on the UI. Decode the JWT to inspect: `echo $TOKEN | cut -d. -f2 | base64 -d | jq .`
...
* *UI makes API calls over HTTP instead of HTTPS*: Set `REGISTRY_API_URL` in the UI env section to the HTTPS URL of the {registry} app. The operator defaults to `http://`, which breaks when authentication is enabled.
...
* *Logout does not work*: Dex does not implement the OIDC `end_session_endpoint`. The UI logout clears the local session but the Dex session may persist. Users may need to clear browser cookies or the Dex session will re-authenticate automatically.
