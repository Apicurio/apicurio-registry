...
* *Field Tags* - Semantic annotation of schema fields (for example, PII, SENSITIVE, EMAIL) _(planned)_
...
NOTE: Field-level tags are planned for a future release and are not yet implemented.
...
NOTE: Governance rules that execute automatically on artifact create/update are planned for a future release.
...
NOTE: The following endpoints are planned for future releases and are not yet implemented.

.Planned data contract REST API endpoints
[cols="1,3,3", options="header"]
|===
|Method |Endpoint |Description
...
NOTE: The `stableDate` and `deprecatedDate` fields are automatically populated when transitioning to STABLE or DEPRECATED status. Invalid transitions (for example, `DEPRECATED` → `STABLE`) return a 409 Conflict response.
...
NOTE: The following configuration properties are planned for future releases and are not yet available.

.Planned data contracts configuration properties
[cols="2,1,2", options="header"]
|===
|Property |Default |Description
...
NOTE: SerDes rule execution configuration is planned for a future release. Contract rules are currently stored in the registry but not executed by serializers/deserializers.
