...
// TODO: https://github.com/Apicurio/apicurio-registry/issues/6919
...
The {operator} uses the `ApicurioRegistry3` custom resource to deploy and manage {registry} instances on
...
IMPORTANT: *Order of evaluation:* The environment variables defined in the `env` field override any environment variables set by the operator. This allows you to both customize configuration while still benefiting from operator-managed settings, and workarounds in case of bugs. You should not use the custom PodTemplateSpec method described below to set environment variables.
