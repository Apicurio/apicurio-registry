...
[role="_abstract"]
This chapter explains how to deploy and configure {registry} instances using the {registry} Operator on
{kubernetes}:
...
== Understanding {registry} Operator deployment

The {registry} Operator uses the `ApicurioRegistry3` custom resource to deploy and manage {registry} instances on
{kubernetes}. The custom resource allows you to declaratively configure all aspects of your {registry} deployment,
...
* `spec.app` - Configures the {registry} backend (REST API) component
* `spec.ui` - Configures the {registry} web console component
...
* You must have already installed the {registry} Operator. See {installing-the-registry-openshift}.

.Procedure

. In the {kubernetes} web console, log in using an account with cluster administrator privileges.

. Change to the {kubernetes-namespace} in which the {registry} Operator is installed. For example, from the
...
----
+
NOTE: Replace the `host` values with appropriate hostnames for your environment. If you are using OpenShift
Routes instead of Ingress, you can leave the `host` field empty and OpenShift will generate a default route.

. Click *Create* and wait for the {registry} deployment to be created.

. Click *Networking* > *Routes* (or *Ingresses*) to access the route for the {registry} web console.
...
* For information on PostgreSQL Operators available on OpenShift, see xref:installing-postgresql-operatorhub_{context}[]
...
`username` to match your database user. The storage `type` should still be set to `postgresql` as {registry}
uses the same SQL storage mechanism for both PostgreSQL and MySQL.
...
with a compaction cleanup policy:
+
[source,yaml]
----
...
----
+
WARNING: You must configure the Kafka topic used by {registry} with a compaction cleanup policy, otherwise data
loss might occur.
...
* For more information on Kafka storage configuration, see xref:setting-up-kafka-streams-storage_{context}[]
in {installing-the-registry-storage-openshift}.
...
** Backend API client (e.g., `registry-client-api`) - Used by the {registry} REST API
** UI client (e.g., `registry-client-ui`) - Used by the {registry} web console
...
{registry-name-full} security in xref:assembly-configuring-registry-security.adoc[].
...
NOTE: The operator extends the provided PodTemplateSpec with default values and applies additional configuration
based on other fields in the `ApicurioRegistry3` CR.
...
* For the complete configuration reference, see xref:assembly-operator-config-reference.adoc[].
