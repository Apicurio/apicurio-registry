...
[role="_abstract"]
{registry} concepts, features, and supported artifact types:
...
Artifacts have both generated and editable metadata. Standard metadata for an artifact includes the following properties:
...
**Artifact versions**
Every artifact is composed of zero or more _artifact versions_. Only artifact versions have actual content and metadata. Versions represent the evolution of the content of an artifact and are immutable. You can think of an artifact as an ordered sequence of versions, typically with the most recent version representing the current schema or API design content.

Artifact versions have both generated and editable metadata. Standard metadata for an artifact version includes the following properties:
...
Groups have both generated and editable metadata. Standard metadata for a group includes the following properties:
...
* https://github.com/Apicurio/apicurio-registry/tree/main/examples/serdes-with-references[{registry} SerDes with references demonstration]
...
NOTE: Some Confluent enterprise features such as Schema Linking, Data Contracts, and client-side encryption (KEK/DEK) are not supported. For details on supported and unsupported features, see xref:getting-started/assembly-confluent-schema-registry-compatibility.adoc[Confluent Schema Registry compatibility API].
...
ifdef::rh-service-registry[]
* link:https://docs.redhat.com/en/documentation/red_hat_build_of_debezium/3.0.8/html/debezium_user_guide/configuring-debezium-connectors-for-your-application#configuring-debezium-connectors-to-use-avro-serialization[Configuring Debezium to use Avro serialization and {registry}]
endif::[]
ifdef::apicurio-registry[]
...
* LLM/AI model schemas and prompt templates
* REST clients

[role="_additional-resources"]
.Additional resources
* link:https://github.com/Apicurio/apicurio-registry/tree/main/examples[{registry} example applications]
...
[role="_additional-resources"]
.Additional resources
* link:https://github.com/Apicurio/apicurio-registry[{registry} source code and Docker images]
* link:https://github.com/Apicurio/apicurio-registry/tree/main/operator[{registry} Operator]
