...
[discrete]
==== Python SerDes support

{registry} also provides a Python SerDes library (`apicurio-registry-serdes`) that supports the same schema types and uses a compatible wire format. This enables interoperability between Java and Python applications using the same {registry} instance. See {python-client-serdes} for details.

[role="_additional-resources"]
.Additional resources
* {kafka-client-serdes}
* {python-client-serdes}
