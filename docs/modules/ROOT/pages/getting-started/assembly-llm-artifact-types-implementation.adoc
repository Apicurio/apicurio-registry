...
[role="_abstract"]
The PROMPT_TEMPLATE artifact type is wired in `schema-util/util-provider/src/main/java/io/apicurio/registry/types/provider/StandardArtifactTypeProviderRegistry.java`, with validators, extractors, and compatibility checkers in `schema-util/common/`.
...
[role="_abstract"]
The MODEL_SCHEMA artifact type is wired in `schema-util/util-provider/src/main/java/io/apicurio/registry/types/provider/StandardArtifactTypeProviderRegistry.java`, with validators, extractors, and compatibility checkers in `schema-util/common/`.
...
A variable placeholder is a word-character name with optional surrounding whitespace, so
`{{name}}` and `{{ name }}` both resolve to the variable `name`. A name containing any other
character, for example `{{user.name}}` or `{{my-var}}`, is not treated as a variable and is left
in the rendered output as literal text. Earlier versions of the render endpoint substituted such
names; it now uses the same grammar as the prompt template validity rule and the MCP prompt
converter, which never did.
