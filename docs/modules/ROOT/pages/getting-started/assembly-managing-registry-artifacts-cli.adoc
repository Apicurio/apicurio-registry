...
* The CLI supports Linux (bash) and macOS (zsh). Windows is not supported yet
...
----
+
This command installs the CLI files to `$HOME/bin` and `$HOME/.apicurio/apicurio-registry-cli`, updates your `~/.bashrc` file (Linux) or `~/.zshrc` file (macOS), and configures shell completions.
