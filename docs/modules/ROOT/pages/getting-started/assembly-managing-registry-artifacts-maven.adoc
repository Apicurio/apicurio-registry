...
== Schema and API artifact registration using the Maven plug-in
...
== Schema and API artifact download using the Maven plug-in
:_mod-docs-content-type: CONCEPT

[role="_abstract"]
You can use the Maven plug-in to download artifacts from {registry}. Downloading artifacts is useful, for example, when generating code from a registered schema.
...
== Schema and API artifact testing using the Maven plug-in
:_mod-docs-content-type: CONCEPT

[role="_abstract"]
You can verify that an artifact can be registered without making any changes, which is useful when rules are configured in {registry}. Testing results in a failure if the artifact content violates any of the configured rules.
...
== Manual artifact reference registration using the {registry} Maven plug-in
:_mod-docs-content-type: CONCEPT
...
== Automatic artifact reference registration using the {registry} Maven plug-in
:_mod-docs-content-type: CONCEPT

[role="_abstract"]
You can configure the Maven plug-in to automatically detect and register all artifact references to artifacts in the same directory. This eliminates the need to manually specify each reference when registering parent artifacts that contain nested references.
...
[role="_abstract"]
When you connect to a TLS-enabled {registry} instance, you can configure the Maven plug-in to use SSL/TLS for secure communication. The plug-in supports various trust store and key store formats, as well as multiple authentication methods.
