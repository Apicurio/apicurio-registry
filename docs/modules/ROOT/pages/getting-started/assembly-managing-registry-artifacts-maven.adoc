...
[role="_abstract"]
You can use the {registry} Maven plug-in to manage schema and API artifacts stored in {registry}.
...
[role="_abstract"]
You can add schema and API artifacts to {registry} during a Maven build by using the `register` execution goal.
...
[role="_abstract"]
You can download schema and API artifacts from {registry} by using the Maven plug-in `download` execution goal, for example, when generating code from a registered schema.
...
[role="_abstract"]
You can verify that an artifact can be registered in {registry} without making any changes, which is useful when validation rules are configured. The test fails if the artifact content violates any configured rules.
...
[role="_abstract"]
You can manually register artifact references in {registry} by specifying each reference in the Maven plug-in configuration. Artifact references allow you to define reusable schema or API artifacts and reference them from multiple locations.
...
The following example shows how to use the {registry} Maven plug-in to manually register an artifact reference to a simple Avro schema artifact stored in {registry}. The example assumes that the following `Exchange` schema artifact has already been created in {registry}:
...
The example then creates a `TradeKey` schema artifact, which includes a reference to the nested `Exchange` schema artifact:
...
[role="_abstract"]
You can configure the Maven plug-in to automatically detect and register all artifact references to artifacts in the same directory, eliminating the need to manually specify each reference.
...
You can specify a single artifact and configure the {registry} Maven plugin to automatically detect all references to artifacts located in the same directory, and to automatically register those references. Automatic reference detection is a Technology Preview feature.
...
The following example shows how to use the Maven plug-in to register an Avro schema and automatically detect and register an artifact reference. The example assumes that the parent `TradeKey` artifact and the nested `Exchange` schema artifact are both available in the same directory.

The `TradeKey` schema is defined as follows:
...
The `Exchange` schema is defined as follows:
...
== TLS and authentication configuration for the Maven plug-in
:_mod-docs-content-type: REFERENCE

[role="_abstract"]
The {registry} Maven plug-in supports SSL/TLS for secure communication with TLS-enabled {registry} instances. The plug-in accepts various trust store and key store formats and multiple authentication methods.
