...
TIP: Alternatively, if you are deploying {registry} on {kubernetes} using the {operator}, you can enable deletion by configuring the `ApicurioRegistry3` custom resource. For more information, see xref:operator-config-reference_{context}[].
