...
* DELETE is a soft deletion: it returns HTTP 200 with status `deleted` and retains
  content/history. Use `include_deleted=true` to inspect it or PATCH status to restore
  it. Permanent deletion is only available through the native v3 API, subject to
  `apicurio.rest.deletion.artifact-version.enabled`; MCP does not bypass that gate.
* Optional PUT returns
...
* Status messages are descriptive, user-editable version metadata. They are not
  authorization or approval state; the lifecycle status is derived from version state.
