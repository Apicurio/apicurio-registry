...
[role="_abstract"]
You can connect Large Language Model (LLM) clients such as Claude Desktop to {registry} by using the Model Context Protocol (MCP) server. The MCP server is a lightweight adapter that uses stdio transport on the LLM client side and connects to {registry} over HTTP or HTTPS by using the REST API. You configure the target {registry} instance by setting the `REGISTRY_URL` environment variable. Through this adapter, you can manage groups, artifacts, versions, and configuration properties by using natural language.
...
[role="_abstract"]
You can use the {registry} MCP server to interact with {registry} through the Model Context Protocol (MCP) from any LLM client. The MCP server acts as an adapter between two protocols: on the LLM client side, it communicates over stdio transport; on the {registry} side, it connects over HTTP or HTTPS by using the REST API. You configure the target {registry} instance by setting the `REGISTRY_URL` environment variable (or the `registry.url` Java system property).
...
NOTE: All Docker commands in this guide also work with Podman. Substitute `podman` for `docker` if you use Podman as your container runtime.
...
----
+
Replace `<registry-url>` with the URL of your {registry} instance, for example `http://localhost:8080`.
+
...
|Verify that {registry} is running and accessible at the configured URL. Check the `REGISTRY_URL` environment variable or `registry.url` property. If you run the MCP server in Docker, ensure that `REGISTRY_URL` points to the correct host and port of your {registry} instance.
