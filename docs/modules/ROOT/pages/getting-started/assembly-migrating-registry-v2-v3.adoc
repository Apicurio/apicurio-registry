...
. Validate the environment before you migrate:
...
. Switch traffic to {registry} 3.x:
...
. Monitor the new deployment and keep {registry-v2} available for rollback.
+
--
...
[role="_abstract"]
Configuration keys in {registry} 3.x adopt a unified `apicurio.` namespace and reorganize authentication, eventing, UI, and storage settings. The following tables map v2 properties to their v3 equivalents, grouped by configuration area, with example migrations.
...
.SQL database storage properties
[id="configuring-storage_{context}",cols="1,1",options="header"]
|===
...
* xref:migrating-registry-configuration_{context}[]
* link:https://www.apicur.io/registry/docs/apicurio-registry/3.1.x/index.html[Official {registry} documentation]
...
All scripts read the following common environment variables:
...
`REGISTRY_API_VERSION`:: API version to use (default: `v2`)
`OUTPUT_FILE`:: Output file path (default: `registry-baseline.txt`)
...
`REGISTRY_API_VERSION`:: API version to use (default: `v3`)
...
`REGISTRY_VERSION`:: Registry version to test (default: `v3`)
...
None. This script uses only the common environment variables.
...
The following list maps common script error messages to checks that resolve them:
