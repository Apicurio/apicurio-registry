...
[role="_abstract"]
Older operator manifests that still use the deprecated `spec.app.sql`, `spec.app.kafkasql`, or `apicurio.apis.date-format*` settings must be updated before upgrading to versions that remove those compatibility shims.

Use the storage configuration fields under `spec.app.storage` and keep API date formatting at the default ISO-8601 format.
