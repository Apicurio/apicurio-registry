...
* Legacy `spec.app.sql`, `spec.app.kafkasql`, and `spec.app.sql.dataSource` compatibility fields were removed in 3.0.7. Use `spec.app.storage.sql` or `spec.app.storage.kafkasql` instead.
* The operator no longer preserves the deprecated `Deprecated*` model classes. Existing manifests should be updated before upgrading the operator.
...
NOTE: The operator provides first-class support for OAuth (OAUTHBEARER) authentication via the `auth` field. For other SASL mechanisms such as SCRAM-SHA-512, the recommended approach is to use the Strimzi Kafka Access Operator via the `kafkaAccessSecretName` field (available starting with {registry} version 3.2.0). Alternatively, you can use custom environment variables via the `spec.app.env` field to configure the Kafka client properties directly. See the xref:getting-started/assembly-config-reference.adoc[] for available `apicurio.kafkasql.*` configuration properties.
