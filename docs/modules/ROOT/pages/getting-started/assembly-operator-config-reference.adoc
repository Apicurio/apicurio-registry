...
= {registry} Operator configuration reference
...
spec:
  app:
    # Configuration for the backend (REST API) component
  ui:
    # Configuration for the web console component
----

[id="operator-app-spec-reference_{context}"]
== Application (backend) configuration reference

The `spec.app` section configures the {registry} backend component, which provides the REST API.
...
|JDBC connection URL
|String
|`jdbc:postgresql://postgresql.my-ns.svc:5432/registry`
...
spec:
  app:
    storage:
      type: postgresql  # Use postgresql type for MySQL as well
      sql:
        dataSource:
...
|SASL mechanism
|String
|`SCRAM-SHA-512`, `PLAIN`, `OAUTHBEARER`
...
* For deployment examples, see xref:assembly-deploying-registry-operator.adoc[].
