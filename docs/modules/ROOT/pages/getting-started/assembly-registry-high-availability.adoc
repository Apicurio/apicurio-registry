...
* *Snapshots topic* - Stores periodic snapshots of the registry state for faster startup, if the snapshotting feature is used. Its named `kafkasql-snapshots` by default and should be configured with similar replication settings as the journal topic.
...
{registry} automatically creates the journal, snapshots, and events topics on startup by default,  if they do not exist. While this is not recommended in a high-availability production scenario, if you want the topics be created automatically, the following environment variables provide the equivalent configuration:
