...
[role="_additional-resources"]
.Additional resources
* xref:configuring-registry-security_{context}[]
...
Because the outbox table has a stable schema, the Avro schema that Debezium registers rarely changes. When you add fields to the outbox event payload, follow the schema evolution patterns to maintain compatibility.

**Schema lifecycle in event-driven context**

The four lifecycle phases apply directly to event-driven architectures. The following list describes how each phase maps to CDC and messaging workflows:
...
A working example that demonstrates Debezium, Kafka, and {registry} working together is available in the {registry} repository.
...
* {managing-registry-artifacts-api}
* xref:registry-rule-reference_{context}[]
* link:https://github.com/Apicurio/apicurio-registry/tree/main/examples/event-driven-architecture[Event-driven architecture example]
