...
----
+
When Kafka internally calls the `configure()` method on the serializer, {registry} automatically detects the injected objects in the configuration map and uses them securely in place of attempting to build new ones from properties.
