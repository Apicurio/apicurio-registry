...
[id="registry-rules-precedence-{context}"]
=== Rule precedence
You can configure the content rules at a global level or at an artifact level. Configured artifact rules override any configured global rules.

NOTE: If no rules are configured for an artifact, the currently configured global rules are applied, if any.

If you want to disable a rule for all artifacts, you must disable the rule at both the artifact level and at the global level. {registry} applies an enabled global rule to any artifacts that have the rule disabled at the artifact level.
