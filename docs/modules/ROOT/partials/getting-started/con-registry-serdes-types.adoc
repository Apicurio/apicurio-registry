...
* `apicurio.registry.avro-datum-provider` - provide a fqn of the AvroDatumProvider implementation; e.g. io.apicurio.registry.utils.serde.avro.ReflectAvroDatumProvider
...
NOTE: the Protobuf deserializer doesn't deserialize to user's exact Protobuf Message implementation,
but to a `DynamicMessage` instance (there is no appropriate API to do otherwise)
