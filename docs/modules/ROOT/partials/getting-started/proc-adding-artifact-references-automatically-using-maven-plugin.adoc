...
You can specify a single parent artifact and configure the Maven plugin to automatically detect all references to child artifacts located in the same directory, and automatically register them.

This section shows a simple example of using the {registry} Maven plug-in to register a parent Avro schema and automatically detect and register an artifact reference to a simple child schema artifact. This example assumes that the parent `TradeKey` and nested `Exchange` schema artifacts are both available in the same directory:
...
<4> Specify the parent artifact group ID. You can specify the `default` group if you do not want to use a unique group ID.
<5> Specify the location of the parent artifact file. The referenced artifacts must also be located in the same directory. 
