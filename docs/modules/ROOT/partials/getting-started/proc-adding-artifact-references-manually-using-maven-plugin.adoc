 ...
 * For an open source example of using the {registry} Maven plug-in to manually register artifact references, see the link:https://github.com/Apicurio/apicurio-registry-examples/tree/main/avro-maven-with-references[avro-maven-with-references demonstration example].
