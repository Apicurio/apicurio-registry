...
This example then creates a `TradeKey` schema artifact, which includes an artifact reference to the nested `Exchange` schema artifact:
...
.Procedure
* Update your Maven `pom.xml` file to use the `apicurio-registry-maven-plugin` to register the `TradeKey` schema, which includes a nested artifact reference to the `Exchange` schema as follows:
+
[source,xml]
----
<plugin>
