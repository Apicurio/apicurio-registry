...
= Configuring {registry} authentication and authorization using {keycloak}
...
[NOTE]
====
Alternatively, for details on how to automatically configure these settings using the 
ifdef::apicurio-registry[]
{keycloak} Operator, see the link:https://www.apicur.io/registry/docs/apicurio-registry-operator/{operator-version}/assembly-registry-maintenance.html#registry-security-keycloak[{registry} Operator documentation].
endif::[]
ifdef::rh-service-registry[]
{keycloak} Operator Technology Preview, see xref:registry-security-keycloak[]. 
endif::[]
====

You can enable authentication for the {registry} web console and core REST API using {keycloak} based on OAuth using OpenID Connect (OIDC). HTTP basic authentication is also suported for development purposes. The same {keycloak} realm and users are federated across the {registry} web console and core REST API using Open ID Connect so that you only require one set of credentials.
...
.Prerequisites
* {keycloak} is installed and running. For details on using {keycloak}, see
ifdef::apicurio-registry[]
...
. In the {keycloak} Admin Console, create a {keycloak} realm for {registry}. By default, {registry} expects a realm name of `registry`. For more details on creating {keycloak} realms, see
ifdef::apicurio-registry[]
link:https://www.keycloak.org/getting-started[Getting Started with {keycloak}]. 
endif::[]
ifdef::rh-service-registry[]
