...
[role="_abstract"]
This section shows a simple curl-based example of using the core registry v2 REST API to export and import existing registry data in `.zip` format from one {registry} instance to another. For example, this is useful when migrating data between {registry} instances. All of the artifact data contained in the {registry} instance is exported in the `.zip` file.

ifdef::rh-openshift-sr[]
IMPORTANT: You can import {registry} data that has previously been exported from another {product-long-registry} instance. You cannot currently import {registry} data from a {org-name} Integration {registry} instance.   
endif::[]
