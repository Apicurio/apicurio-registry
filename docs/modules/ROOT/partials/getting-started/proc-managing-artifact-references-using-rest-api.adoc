...
.Dereferencing artifact content

There are some cases where storing artifact content with the referenced content inline might be helpful. For these cases, the Core Registry API v2 adds support for the `dereference` parameter in certain operations. 
