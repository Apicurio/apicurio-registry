...
[discrete]
=== {registry} HTTP basic client credentials token expiry 

You can also configure the HTTP basic client credentials token expiry time. By default, when using HTTP basic authentication, {registry} caches the JWT token, and does not issue a new token when there is no need. You can configure the cache expiry time for JWT tokens, which is set to 10 mins by default. 

When using {keycloak}, it is best to set configuration variable to your {keycloak} JWT expiry time minus 1 minute. For example, if you have the expiration time set to `5` mins in {keycloak}, you should set the following configuration option to `4` mins:

.Configuration for HTTP basic client credentials token expiry
[%header,cols="4,4,1,1"]
|===
|Environment variable
|Java system property
