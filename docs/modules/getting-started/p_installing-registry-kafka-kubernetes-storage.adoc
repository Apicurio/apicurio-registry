...
** `REGISTRY_ROUTE`: The name of route for {registry} in your OpenShift cluster (for example, `my-registry.example.com`).
+
For example: 
+
[source,bash]
----
$ oc new-app service-registry-template.yml -p KAFKA_BOOTSTRAP_SERVERS=my-cluster-kafka-bootstrap:9092 -p REGISTRY_ROUTE=my-registry.example.com
----
+
...
Deploying template "myproject/service-registry" for "service-registry-template.yml" to project myproject
    ...
    * With parameters:
       * Registry Route Name=my-registry.example.com
...
--> Success
   Access your application via route 'my-registry.example.com' 
...
** `REGISTRY_ROUTE`: The name of the route for {registry} in your OpenShift cluster (for example, `my-registry.example.com`).
+
For example: 
+
[source,bash]
----
$ oc new-app apicurio-registry-template-kafka.yml -p KAFKA_BOOTSTRAP_SERVERS=my-cluster-kafka-bootstrap:9092 -p REGISTRY_ROUTE=my-registry.example.com
----
endif::[]
...
[source,bash]
----
$ curl -X POST -H "Content-Type: application/json" --data '{"id": "Procurement Invoice", "name": "My Invoice", "description": "My invoice description", "type": "AVRO", "version": 1}' my-registry.example.com/artifacts 
----
