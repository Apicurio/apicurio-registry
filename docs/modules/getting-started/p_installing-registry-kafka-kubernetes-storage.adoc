...
* You must have already installed Red Hat AMQ Streams on your OpenShift cluster. For details, see link:https://access.redhat.com/documentation/en-us/red_hat_amq/7.5/html/using_amq_streams_on_openshift/getting-started-str?lb_target=stage[Getting Started with AMQ Streams].  
...
. Get the IP address of your Kafka cluster bootstrap service. For example:
+
[source,bash]
----
$ oc get services | grep .*-bootstrap
my-cluster-kafka-bootstrap  ClusterIP  192.0.2.1  <none>  9091/TCP  30m
----
ifdef::rh-service-registry[]
...
[source,bash]
----
$ oc new-app service-registry-template.yml -p KAFKA_BOOTSTRAP_SERVERS={KAFKA_CLUSTER_IP_ADDR}:9092
----
+
...
Deploying template "myproject/service-registry" for "service-registry-template.yml" to project myproject
    ...
    * With parameters:
       ...
       * Kafka Bootstrap Servers=192.0.2.1:9092
...
* For more details on installing templates, see the link:https://docs.openshift.com/container-platform/4.2/openshift_images/using-templates.html[OpenShift user documention].
