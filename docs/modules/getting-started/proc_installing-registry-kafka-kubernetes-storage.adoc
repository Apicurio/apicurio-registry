...
** `APPLICATION_SERVER_HOST`: The IP address of your AMQ Streams application server host, which is required in a multi-node Kafka configuration. Defaults to `$(POD_IP)`.
** `APPLICATION_SERVER_PORT`: The port number of your AMQ Streams application server, which is required in a multi-node Kafka configuration. Defaults to `9000`.
+
The following command shows an example:  
+
[source,bash]
...
----
+
You should see output such as the following when complete: 
+
----
...
. Run `oc status` to view your {registry} installation on OpenShift. 
