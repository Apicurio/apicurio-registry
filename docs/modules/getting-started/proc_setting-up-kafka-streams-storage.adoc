...
+
NOTE: If you have already deployed a Kafka cluster in another way, for example, on the command line, you can still use your Kafka cluster with the {registry} Operator, but you must create the required Kafka topics: `storage-topic` and `global-id-topic`.
