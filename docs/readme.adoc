...
== Linting for the downstream modular build

The `getting-started` assemblies are split into modules by the downstream (Red Hat) build, which fails on structure that Antora accepts. CI checks every docs pull request against that split view: `docs/scripts/vale-pipeline.py` splits each changed assembly with `leben.py` and runs vale with the AsciiDocDITA rules over the result, and `docs/scripts/downstream-compat.py` checks the structural contract the splitter relies on. Errors block the merge; warnings are reported only.
...
For prerequisites, options, and the list of rules, see link:scripts/README.md[docs/scripts/README.md].
