...
#
# Usage:
#   ./test-phase1.sh                    # Dev mode (default)
...
# Default to dev mode: quarkus:dev on 8080, Varnish on 6081
VARNISH_URL="${VARNISH_URL:-http://localhost:6081}"
...
check_services() {
    ...
    # Detect mode
    if [ "$VARNISH_URL" = "http://localhost:6081" ]; then
        print_info "Mode: Dev (quarkus:dev + docker-compose-dev.yml)"
    elif [ "$VARNISH_URL" = "http://localhost:8080" ]; then
        print_info "Mode: Full Stack (docker-compose.yml)"
    else
        print_info "Mode: Custom"
    fi
    echo ""
