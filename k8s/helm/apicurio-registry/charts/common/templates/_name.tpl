...
Usage: 'name: "{{- template "common.name" . -}}"'
  ...
  {{- $base := default (printf "%s" .Chart.Name) .Values.nameOverride -}}
...
{{- /*
common.name.unique adds a random suffix to the unique name.

This takes the same parameters as common.name

*/ -}}
{{- define "common.name.unique" -}}
  {{ template "common.name" . }}-{{ randAlphaNum 7 | lower }}
{{- end }}
