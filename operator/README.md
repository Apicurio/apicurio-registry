...
**Runtime Kill Switch:** To disable application-level compression at runtime without rebuilding the image, set `APICURIO_REST_COMPRESSION_ENABLED=false` in your CR's `env:` block. This bypasses the Quarkus compression interceptor entirely, regardless of the baked-in Quarkus settings.
