...
public class StatusManager {
    ...
    public ApicurioRegistry3 applyStatus(ApicurioRegistry3 primary, Context<ApicurioRegistry3> context) {
        ...
        var previousObservedGeneration = primary.getStatus() != null ? primary.getStatus().getObservedGeneration() : null;
        var currentObservedGeneration = primary.getMetadata().getGeneration();

        // TODO: Try to only update the status when the generation has changed.
        if (!Objects.equals(previousObservedGeneration, currentObservedGeneration)) {
            status.setObservedGeneration(currentObservedGeneration);
        }
