...
Since version `1.0.0`, the {operator} project is distributed with an additional archive file containing installation files for the Operator, with example `ApicurioRegistry3` custom resource files. In addition, full {operator} documentation and license information are included. 
