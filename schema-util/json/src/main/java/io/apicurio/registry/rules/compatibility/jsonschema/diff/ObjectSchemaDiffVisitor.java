...
public class ObjectSchemaDiffVisitor extends JsonSchemaWrapperVisitor {
    ...
    public void visitPropertySchemas(Map<String, SchemaWrapper> propertySchemas) {
        ...
        List<SchemaWrapper> removedRequiredPropertySchemas = new ArrayList<>();
        List<SchemaWrapper> removedOptionalPropertySchemas = new ArrayList<>();
        for (String propertySchemaName: allPropertySchemaNames) {
            ...
            if (!existInOriginal && existInUpdated) {
                ...
                boolean isRequired = original.getRequiredProperties().contains(propertySchemaName);
                Schema property = original.getPropertySchemas().get(propertySchemaName);
                // removing properties
                if (isRequired) {
                    removedRequiredPropertySchemas.add(wrap(property));
                } else {
                    removedOptionalPropertySchemas.add(wrap(property));
                }
            } else if (existInOriginal && existInUpdated) {
        ...
        if (!removedOptionalPropertySchemas.isEmpty()) {
            diffSubSchemasRemoved(ctx.sub("propertySchemasRemoved"), removedOptionalPropertySchemas,
        ...
        if (!removedRequiredPropertySchemas.isEmpty()) {
            ctx.addDifference(OBJECT_TYPE_REQUIRED_PROPERTY_SCHEMAS_NARROWED, removedRequiredPropertySchemas, ctx.sub("propertySchemasRemoved"));
        }
