...
public class OpenApiCompatibilityChecker extends AbstractCompatibilityChecker<Object> {
    ...
    @Override
    protected Set<Object> isBackwardsCompatibleWith(final String existing, final String proposed, final Map<String, TypedContent> resolvedReferences) {
        ...
        List<ChangedSchema> incompatibleSchemas = diff.getChangedSchemas().stream()
            .filter(Objects::nonNull)
            .filter(Changed::isIncompatible)
            .toList();

        List<Endpoint> missingEndpoints = diff.getMissingEndpoints().stream()
            .filter(Objects::nonNull)
            .toList();
