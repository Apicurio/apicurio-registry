...
public class HeaderUtils {
    ...
    private final String groupIdHeaderName;
    ...
    public HeaderUtils(Map<String, Object> configs, boolean isKey) {
        if (isKey) {
            groupIdHeaderName = (String) configs.getOrDefault(SerdeConfigKeys.HEADER_KEY_GROUP_ID_OVERRIDE_NAME, SerdeHeaders.HEADER_KEY_GROUP_ID);
            artifactIdHeaderName = (String) configs.getOrDefault(SerdeConfigKeys.HEADER_KEY_ARTIFACT_ID_OVERRIDE_NAME, SerdeHeaders.HEADER_KEY_ARTIFACT_ID);
            globalIdHeaderName = (String) configs.getOrDefault(SerdeConfigKeys.HEADER_KEY_GLOBAL_ID_OVERRIDE_NAME, SerdeHeaders.HEADER_KEY_GLOBAL_ID);
            versionHeaderName = (String) configs.getOrDefault(SerdeConfigKeys.HEADER_KEY_VERSION_OVERRIDE_NAME, SerdeHeaders.HEADER_KEY_VERSION);
            encodingHeaderName = SerdeHeaders.HEADER_KEY_ENCODING;
            messageTypeHeaderName = (String) configs.getOrDefault(SerdeConfigKeys.HEADER_KEY_MESSAGE_TYPE_OVERRIDE_NAME, SerdeHeaders.HEADER_KEY_MESSAGE_TYPE);

        } else {
            groupIdHeaderName = (String) configs.getOrDefault(SerdeConfigKeys.HEADER_VALUE_GROUP_ID_OVERRIDE_NAME, SerdeHeaders.HEADER_VALUE_GROUP_ID);
            artifactIdHeaderName = (String) configs.getOrDefault(SerdeConfigKeys.HEADER_VALUE_ARTIFACT_ID_OVERRIDE_NAME, SerdeHeaders.HEADER_VALUE_ARTIFACT_ID);
            globalIdHeaderName = (String) configs.getOrDefault(SerdeConfigKeys.HEADER_VALUE_GLOBAL_ID_OVERRIDE_NAME, SerdeHeaders.HEADER_VALUE_GLOBAL_ID);
            versionHeaderName = (String) configs.getOrDefault(SerdeConfigKeys.HEADER_VALUE_VERSION_OVERRIDE_NAME, SerdeHeaders.HEADER_VALUE_VERSION);
            encodingHeaderName = SerdeHeaders.HEADER_VALUE_ENCODING;
            messageTypeHeaderName = (String) configs.getOrDefault(SerdeConfigKeys.HEADER_VALUE_MESSAGE_TYPE_OVERRIDE_NAME, SerdeHeaders.HEADER_VALUE_MESSAGE_TYPE);
        }
    }
     ...
     */
    public void addSchemaHeaders(Headers headers, String groupId, String artifactId, String version, long globalId) {
        ...
        headers.add(groupIdHeaderName, IoUtil.toBytes(groupId));
        headers.add(artifactIdHeaderName, IoUtil.toBytes(artifactId));
        headers.add(versionHeaderName, IoUtil.toBytes(version));
    ...
    public String getGroupId(Headers headers) {
        Header header = headers.lastHeader(groupIdHeaderName);
        if (header == null) {
            throw new RuntimeException("GroupId not found in headers.");
        }
