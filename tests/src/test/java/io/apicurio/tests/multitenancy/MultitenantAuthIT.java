...
import io.apicurio.tests.RegistryBaseIT;
import io.apicurio.tests.RegistryFacade;

@Tag(Constants.MULTITENANCY)
public class MultitenantAuthIT extends RegistryBaseIT {
    ...
    public void testSecuredMultitenantRegistry() throws Exception {
        ...
        RegistryRestClient clientTenant1 = createAndGetTenantAdminClient();

        RegistryRestClient clientTenant2 = createAndGetTenantAdminClient();
    ...
    private void performTenantAdminOperations(RegistryRestClient client) throws Exception {
        assertTrue(client.listArtifacts().size() == 0);
        ...
        assertTrue(client.listArtifacts().size() == 1);
    ...
    private void cleanTenantArtifacts(RegistryRestClient client) throws Exception {
        ...
        TestUtils.retry(() -> assertTrue(client.listArtifacts().isEmpty()));
    }

    private RegistryRestClient createAndGetTenantAdminClient() {
        ...
        RegistryRestClient client  = RegistryRestClientFactory.create(tenantInfo.getTenantAppUrl() + "/api", Collections.emptyMap(), auth);
        return client;
    }
