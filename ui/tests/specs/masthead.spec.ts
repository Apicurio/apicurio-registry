...
const EXPECTED_LOGO_SRC: string = process.env["EXPECTED_LOGO_SRC"] || (IS_DOWNSTREAM ? "/Logo-Red_Hat-A-Standard-RGB.svg" : "/apicurio_registry_logo_default.svg");
...
test("Masthead - Logo verification", async ({ page }) => {
    ...
    const logoImage = page.locator(".pf-v6-c-brand");
    ...
    // Verify the alt text
    await expect(logoImage).toHaveAttribute("alt", EXPECTED_ALT);
});
