...
public class ResponseUtils {
    ...
    public static byte[] toBytesSchema(InputStream response) {
        return IoUtil.toBytes(response);
    }

    public static Schema toAvroSchema(Response response) {
        Object responseEntity = response.getEntity();
        if (responseEntity instanceof InputStream) {
            return toAvroSchema((InputStream) responseEntity);
        } else {
            return AvroSchemaUtils.parse(response.readEntity(String.class));
        }
    }

    public static Schema toAvroSchema(InputStream response) {
        return AvroSchemaUtils.parse(IoUtil.toString(response));
    }
